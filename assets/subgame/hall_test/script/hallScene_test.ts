...
import * as hallWebSocket from '../../../base/common/hallWebSocket';
import * as subGameList from './hqqGameList_test';
import { gameBtnState } from '../../../base/app/appGlobal';
import { DEBUG } from 'cc/env';
const { ccclass, property } = cc._decorator;

@ccclass
export class hallScene_test extends cc.Component {
    public hbslFont: cc.LabelAtlas | null = null;

    private test: boolean = true;
    private menuBtnInfoList: Array<string> = [];
    private fuxinMenuBtnInfoList: Array<string> = [];
    private judingMenuBtnInfoList: Array<string> = [];
    private subGameBtnMap: object = {};
    private subGameBtnArr: Array<hqqNode> = [];
    private isSubBtnClicked: boolean = false;
    private btngonggao: cc.Node = null;
    private btngonggaopoint: cc.Node = null;
    private topbubble: cc.Node = null;
    private btnhby: cc.Node = null;
    private HBYTimeLabel: cc.Label = null;
    private btnjfcj: cc.Node = null;
    private jchdRedPoint: cc.Node = null;
    private btnshouyi: cc.Node = null;
    private shouyipoint: cc.Node = null;
    private btnkefu: cc.Node = null;
    private kefupoint: cc.Node = null;
    private namelabel: cc.Node = null;
    private idlabel: cc.Node = null;
    private head: cc.Node = null;
    private coinlabel: cc.Node = null;
    private suggameScrollView: cc.ScrollView = null;
    private itembtn: cc.Node = null;
    private youkeicon: cc.Node = null;
    private anilayer: cc.Node = null;
    private noticepanel: cc.Node = null;
    private menuBtnListNode: cc.Node = null;
    private btnMenuSelect: cc.Node = null;
    private beauty: cc.Node = null;
    private btnlist: Array<hqqNode> = [];
    private radii: number = null;
    private rad: number = null;
    private isMenuRun: boolean = false;
    private subGameList: Array<hqqSubgame> = [];
    private sgjxhr: XMLHttpRequest = null;
    private hbslxhr: XMLHttpRequest = null;
    private hbldxhr: XMLHttpRequest = null;
    private tempindex: number = null;
    private tempindex2: number = null;
    private bindbtn: cc.Node = null;
    private selectType: string = "";
    private scrollviewcotentX: number = undefined;
    private xinlongad1: cc.Node = null;
    private xinlongad2: cc.Node = null;
    private xinlongad3: cc.Node = null;
    private ninetwothirdlist: Object = {};
    private ninetwoleft: cc.Node = null;
    private ninetworight: cc.Node = null;
    private xinshengad1: cc.Node = null;
    private xinshengad2: cc.Node = null;

    getClassName() {
        return "hallScene_test";
    }

    onLoad() {
        hqq.eventMgr.unregister(hqq.eventMgr.showBiglayer, "hqqViewCtr");
        hqq.eventMgr.unregister(hqq.eventMgr.showSamlllayer, "hqqViewCtr");
        hqq.eventMgr.unregister(hqq.eventMgr.showNotice, "hqqViewCtr");
        hqq.eventMgr.unregister(hqq.eventMgr.showRegister, "hqqViewCtr");
        hqq.eventMgr.unregister(hqq.eventMgr.showPerson, "hqqViewCtr");

        hqq.eventMgr.register(hqq.eventMgr.showBiglayer, this.getClassName(), subGameList.showBigsublayer.bind(subGameList))
        hqq.eventMgr.register(hqq.eventMgr.showSamlllayer, this.getClassName(), subGameList.showSamlllayer.bind(subGameList))
        hqq.eventMgr.register(hqq.eventMgr.showNotice, this.getClassName(), subGameList.showNoticelayer.bind(subGameList))
        // hqq.eventMgr.register(hqq.eventMgr.showRegister, this.getClassName(), subGameList.showRegisterlayer.bind(subGameList))
        hqq.eventMgr.register(hqq.eventMgr.showPerson, this.getClassName(), subGameList.showPerson.bind(subGameList))
        ...
        if (hqq.app.pinpai == "fuxin") {
            ...
            this.menuBtnInfoList = ["all", "changyong", "duizhan", "tiyu", "shixun", "touzhu", "dianzi"];
        } else if (hqq.isOtherGame ||hqq.app.isB2BList()) {
            this.selectType = "remen";
            this.menuBtnInfoList = ["remen", "touzhu", "duizhan", "jieji"]
        }
        ...
        hqq["hall_test"].load("labelatlas/num2", cc.LabelAtlas, (error: Error, labelatlas: cc.LabelAtlas) => {
            if (error) {
                cc.log(error);
                return;
            }
            this.hbslFont = labelatlas;
            this.UILoad();
            // if (hqq.app.pinpai == "fuxin") {
            ...
            //     })
            //     hqq.eventMgr.dispatch(hqq.eventMgr.showNetStateNode, { parent: pn, position: { x: 0, y: 0 } })
            // } else if (hqq.app.pinpai == "tetu" || hqq.app.pinpai == "xingyao" ||
            //     hqq.app.pinpai == "modeng" || hqq.app.pinpai == "xingcai" ||
            //     hqq.app.pinpai == "xingbyx" || hqq.app.pinpai == "kgame") {
            //     let pn = hqq.addNode(this.node, {
            //         widget: { right: 50, bottom: 50 }
            //     })
            //     hqq.eventMgr.dispatch(hqq.eventMgr.showNetStateNode, { parent: pn, position: { x: 0, y: 0 } })
            // } else {
            //     let pn = hqq.addNode(this.node, {
            //         widget: { left: 400, top: 40 }, width: 0, height: 0
            //     })
            //     cc.log("=================== showNetStateNode this.node=", this.node, " pn=", pn)
            //     hqq.eventMgr.dispatch(hqq.eventMgr.showNetStateNode, { parent: pn, position: { x: 0, y: 0 } })
            // }
            ...
            if (hqq.app.needShowFree) {
                ...
                if (hqq.gameGlobal.player.phonenum == "") {
                    ...
                    //     return
                    // }
                    if (!hqq.app.isB2BList()) {
                        hqq.eventMgr.dispatch(hqq.eventMgr.showRegister, {
                            exitfunc: () => {
                                //富鑫公告没用到先注解
                                // if( hqq.app.pinpai == "fuxin")
                                // {
    ...
    UILoad() {
        ...
        if (hqq.app.pinpai == "fuxin") {
            hqq["hall_fuxin"].load("prefab/fuxin", cc.Prefab, (err, prefab) => {
                ...
                this.btnMenuSelect.x = this.btnMenuSelect.getComponent(cc.UITransform).width / 2
                ...
                // hqq.addNode(background, { normal: "base/language/" + hqq.language + "/img/vip_diamond", widget: { horizontalCenter: 220, bottom: -10 }, callback: "toogleManyCai", script: this });
            ...
            hqq["hall_xingui"].load("prefab/xingui", cc.Prefab, (err, prefab) => {
                ...
                this.btnlist = [];
                for (let i = 0; i < menu.getChildByName("btnlist").children.length; i++) {
                    this.btnlist.push(<hqqNode>menu.getChildByName("btnlist").children[i]);
                }
                this.radii = 260
                ...
                hqq.setSprite(goldback, { Res: hqq["hall_" + hqq.app.pinpai], path: hpath + "img/d_jb", widget: { top: 18, left: (350 * (cc.view.getVisibleSize().width / 1334)) }, size: { width: 300, height: 70 } })
                ...
                // hqq.addNode(background, { normal: "base/language/" + hqq.language + "/img/vip_diamond", widget: { horizontalCenter: 300, bottom: -10 }, callback: "toogleManyCai", script: this });
            ...
            hqq["hall_xinsheng"].load("prefab/xinsheng", cc.Prefab, (err, prefab) => {
                ...
                background.addChildEx(xinsheng);
                ...
                hqq.setBtn(btnjchd, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnjchd", widget: { top: 15, right: 240, closeleft: true, closebottom: true }, callback: "onClickJCHD", script: this })
                hqq.setBtn(btnqmdl, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnqmdl", widget: { top: 15, right: 115, closeleft: true, closebottom: true }, callback: "onClickQMDL", script: this })
                hqq.setBtn(this.btngonggao, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btngonggao", widget: { top: 15, right: 40, closeleft: true, closebottom: true }, callback: "onClickGG", script: this })

                hqq.setBtn(this.btnshouyi, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnshouyi", widget: { left: 0, bottom: 0, closeright: true, closetop: true }, callback: "onClickSY", script: this })
                hqq.setBtn(this.btnkefu, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnkefu", widget: { left: 188, bottom: 0, closeright: true, closetop: true }, callback: "onClickKF", script: this })
                hqq.setBtn(btnmfjb, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnmfjb", widget: { left: 376, bottom: 0, closeright: true, closetop: true }, callback: "onClickMFJB", script: this })
                hqq.setBtn(btnchongzhi, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnchongzhi", widget: { right: 0, bottom: 0, closeleft: true, closetop: true }, callback: "onClickCC", script: this })
                ...
                // hqq.addNode(background, { normal: "base/language/" + hqq.language + "/img/vip_diamond", widget: { horizontalCenter: 150, bottom: -10 }, callback: "toogleManyCai", script: this });
                // hqq.addNode(background, { normal: "base/language/"+hqq.language+"/img/vip_diamond", widget:{horizontalCenter:150,bottom:-10} ,callback:"toogleManyCai",script:this});
            ...
            hqq["hall_xinlong"].load("prefab/xinlong", cc.Prefab, (err, prefab) => {
                ...
                hqq.setWidget(subgameview, { left: 250, top: 100, bottom: 100 })
                hqq.setLabel(this.namelabel, { string: "", position: { x: -100, y: 20 }, fontSize: 25 })
                hqq.setLabel(this.coinlabel, { string: "", fontSize: 25 })
                ...
                let bdk = hqq.addNode(namebg, { Res: hqq["hall_" + hqq.app.pinpai], normal: hpath + "img/bdk", x: -32, y: -20, callback: "onClickPlayerBtn", script: this })
                hqq.addNode(bdk, { Res: hqq["hall_" + hqq.app.pinpai], path: hlpath + "grxx2", y: 3.354 })
                hqq.setSprite(goldback, { Res: hqq["hall_" + hqq.app.pinpai], path: hpath + "img/GoldBG", widget: { left: 405, top: 5 }/*, size: { width: 200 }*/ })
                hqq.setSprite(jinbi, { path: "base/img/sp_coin2", widget: { horizontalCenter: -100 } })
                //hqq.setBtn(btncopy, { Res:hqq["hall_"+hqq.app.pinpai],normal: hpath + "img/btncopy", widget: { horizontalCenter: 81 }, callback: "onClickPlayerBtn", script: this })
                btncopy.active = false;
                hqq.setBtn(btnplus, { Res: hqq["hall_" + hqq.app.pinpai], normal: "xinlong/img/btn_refresh2", widget: { horizontalCenter: 95 }, callback: "onClickGetGold", script: this, size: { height: 54, width: 54 } })

                // hqq.setBtn(this.btnhby, { Res:hqq["hall_"+hqq.app.pinpai],normal: hlpath + "btnhby", widget: { top: 15, right: 460 }, callback: "onClickHBY", script: this })
                // hqq.setBtn(this.btnjfcj, { Res:hqq["hall_"+hqq.app.pinpai],normal: hlpath + "btnjfcj", widget: { top: 15, right: 355 }, callback: "onClickJFCJ", script: this })
                hqq.setBtn(btnjchd, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnjchd", widget: { top: 8, right: 125, closeleft: true, closebottom: true }, callback: "onClickJCHD", script: this })
                hqq.setBtn(btnqmdl, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnqmdl", widget: { top: 8, right: 230, closeleft: true, closebottom: true }, callback: "onClickQMDL", script: this })
                ...
                hqq.setBtn(this.btnshouyi, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnshouyi", widget: { left: 45, bottom: 10, closeright: true, closetop: true }, callback: "onClickSY", script: this })
                hqq.setBtn(this.btnkefu, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnkefu", widget: { left: 275, bottom: 5, closeright: true, closetop: true }, callback: "onClickKF", script: this })
                hqq.setBtn(btnmfjb, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnmfjb", widget: { left: 690, bottom: 10, closeright: true, closetop: true }, callback: "onClickMFJB", script: this })
                hqq.setBtn(btnchongzhi, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath + "btnchongzhi", widget: { right: 63, bottom: 5, closeleft: true, closetop: true }, callback: "onClickCC", script: this })
                ...
                this.xinlongad1 = hqq.addNode(cc.find("Canvas/hallback/xinlong/renwu"), { Res: hqq["hall_" + hqq.app.pinpai], path: hlpath + "lbt1" });
                this.xinlongad2 = hqq.addNode(cc.find("Canvas/hallback/xinlong/renwu"), { Res: hqq["hall_" + hqq.app.pinpai], path: hlpath + "lbt2" });
                this.xinlongad3 = hqq.addNode(cc.find("Canvas/hallback/xinlong/renwu"), { Res: hqq["hall_" + hqq.app.pinpai], path: hlpath + "lbt3" });
                ...
                let scroll = panel.getChildByName("noticescroll")
                scroll.x = 10
                hqq.setSprite(panel, { Res: hqq["hall_" + hqq.app.pinpai], path: hpath + "sp_trumpet_bg", y: -120 })
                hqq.setSkeleton(panel.getChildByName("sp_trumpet"), { Res: hqq["hall_" + hqq.app.pinpai], path: "xinlong/jd_ani_trumpet", x: -410, aniname: "animation", loop: true })
                this.UILoadFinished();
            })
            ...
            hqq["hall_test"].load("test/prefab/test", cc.Prefab, (err, prefab) => {
                ...
                hqq.setBtn(btnjchd, { Res: hqq["hall_test"], path: hrpath + "icon_jchd", aniname: "Animation1", loop: true, widget: { top: -5, right: 165, closeleft: true, closebottom: true }, callback: "onClickJCHD", script: this })
                hqq.setBtn(btnqmdl, { Res: hqq["hall_test"], path: hrpath + "quanming", aniname: "dating_qmdl", loop: true, widget: { top: 5, right: 70, closeleft: true, closebottom: true }, callback: "onClickQMDL", script: this })
                hqq.setBtn(this.btngonggao, { Res: hqq["hall_test"], normal: hlpath + "icon_gonggao", widget: { top: 15, right: 18, closeleft: true, closebottom: true }, callback: "onClickGG", script: this })

                hqq.setBtn(this.btnkefu, { Res: hqq["hall_test"], normal: hlpath + "icon_chat2", widget: { left: 70, bottom: 30, closeright: true, closetop: true }, callback: "onClickKF", script: this })
                hqq.setBtn(this.btnshouyi, { Res: hqq["hall_test"], normal: hlpath + "icon_duihuan", widget: { left: 245, bottom: 30, closeright: true, closetop: true }, callback: "onClickSY", script: this })
                hqq.setBtn(btnmfjb, { Res: hqq["hall_test"], normal: hlpath + "btnmfjb", widget: { left: 445, bottom: 30, closerightt: true, closetop: true }, callback: "onClickMFJB", script: this })
                hqq.setBtn(btnchongzhi, { Res: hqq["hall_test"], path: hrpath + "chongzhi", aniname: "dating_chongzhi", loop: true, widget: { right: 28, bottom: -130, closeleft: true, closetop: true }, size: { width: 385, height: 255 }, callback: "onClickCC", script: this })
                ...
                // hqq.addNode(background, { normal: "base/language/" + hqq.language + "/img/vip_diamond", widget: { horizontalCenter: 135, bottom: 20 }, callback: "toogleManyCai", script: this });
                hqq.addNode(background, { Res: hqq["hall_test"], path: hrpath + "xinhao/logo_dating", widget: { horizontalCenter: 60, top: 10 } });
                this.UILoadFinished();
            })
        } else if (hqq.app.pinpai == "juding") {
            hqq["hall_juding"].load("prefab/juding", cc.Prefab, (err, prefab) => {
                ...
                hqq.setNode(this.head, { width: 76, height: 76 })
            ...
            hqq["hall_ninetwo"].load("prefab/ninetwo", cc.Prefab, (err, prefab) => {
                ...
                this.head = hqq.addNode(touxiang, { height: 74, width: 74, x: -2, y: 4, zIndex: 0 });
                this.head.addComponent(cc.Sprite);
                hqq.addNode(touxiang, { Res: hqq["hall_" + hqq.app.pinpai], normal: hlpath2 + "dating_headFrame", aniname: "touxiankuang_small", loop: true, timeScale: 0.95, height: 76, width: 76, callback: "onClickPlayerBtn", script: this, zIndex: 0 })
                ...
                hqq.addNode(this.bindbtn, { string: "bindbtn", fontSize: 20, lineHeight: 25, color: cc.color(59, 31, 11, 255), bold: true, y: -2 });
                if (hqq.gameGlobal.player.phonenum != "") {
                ...
                // hqq.addNode(background, { normal: "base/language/" + hqq.language + "/img/vip_diamond", widget: { horizontalCenter: 220, bottom: 0 }, callback: "toogleManyCai", script: this });
                hqq.addNode(background, { Res: hqq["hall_" + hqq.app.pinpai], skeleton: hlpath2 + "dating_title/", aniname: "animation", loop: true, timeScale: 0.95, widget: { horizontalCenter: 60, top: 0 } });
                hqq.addNode(background, { Res: hqq["hall_" + hqq.app.pinpai], path: hpath + "xian", widget: { bottom: 5, left: 270 } })
                hqq.addNode(background, { Res: hqq["hall_" + hqq.app.pinpai], path: hpath + "xian", widget: { bottom: 5, right: 390 } })
                hqq.setWidget(subgameview, { left: 310, top: 100, bottom: 60 })
                // hqq.addNode(background,{Res:hqq["hall_"+hqq.app.pinpai],normal:hlpath2+"dating_icon_bigPot/",aniname:"animation",loop:true,timeScale:0.95,widget:{left:400,top:15},callback:"onClickDJC",script:this});
                let tempsetting = hqq.addNode(background, { normal: "", width: 54, height: 74, widget: { right: 15, top: 10 }, callback: "onClickSheZhi", script: this });
                ...
                this.noticepanel.active = true
                this.ninetwoleft = hqq.addNode(background, { Res: hqq["hall_" + hqq.app.pinpai], normal: "ninetwo/ani_dating_arrow_20211129", aniname: "left", loop: true, widget: { left: 300 }, callback: "onClickNineTwoScrollViewLeft", script: this });
                this.ninetworight = hqq.addNode(background, { Res: hqq["hall_" + hqq.app.pinpai], normal: "ninetwo/ani_dating_arrow_20211129", aniname: "right", loop: true, widget: { right: 30 }, callback: "onClickNineTwoScrollViewRight", script: this });
                this.UILoadFinished();
            })
        } else {
            let hlpath = "language/" + hqq.language + "/img/"
            ...
            if (hqq.isOtherGame) {
                hqq.setSprite(background, { Res: hqq["hall_test"], path: "tetu/bigimg/bj" })
                hqq.setSprite(namebg, { Res: hqq["hall_test"], path: hpath + "img/namebg", widget: { left: 120, top: 0, target: background.parent }, width: 350 })
                hqq.setLabel(this.namelabel, { string: "", widget: { closeleft: true }, x: -150, anchorX: 0, horizontalAlign: cc.HorizontalTextAlignment.LEFT });
                let idbg = cc.instantiate(namebg);
                idbg.name = "idbg";
                namebg.parent.addChildEx(idbg);
                this.idlabel = idbg.getChildByName("namelabel") // ID文本
                hqq.setSprite(idbg, { Res: hqq["hall_test"], path: hpath + "img/namebg", widget: { left: 120, top: 50, target: background.parent }, width: 350 })
                this.idlabel.active = false;
                idbg.active = false;
            } else {
                hqq.setSprite(background, { Res: hqq["hall_test"], path: hpath + "bigimg/roombgmin" })
            ...
            hqq.setBtn(this.btnkefu, { Res: hqq["hall_test"], normal: hlpath + "icon_chat2", widget: { left: 70, bottom: 30, closeright: true, closetop: true }, callback: "onClickKF", script: this })
            hqq.setBtn(this.btnshouyi, { Res: hqq["hall_test"], normal: hlpath + "icon_duihuan", widget: { left: 245, bottom: 30, closeright: true, closetop: true }, callback: "onClickSY", script: this })
            hqq.setBtn(btnmfjb, { Res: hqq["hall_test"], normal: hlpath + "btnmfjb", widget: { left: 445, bottom: 30, closeright: true, closetop: true }, callback: "onClickMFJB", script: this })
            hqq.setBtn(btnchongzhi, { Res: hqq["hall_test"], path: hrpath + "chongzhi", aniname: "dating_chongzhi", loop: true, widget: { right: 28, bottom: -130, closeleft: true, closetop: true }, size: { width: 385, height: 255 }, callback: "onClickCC", script: this })
            if (hqq.app.pinpai == "yuyu" || hqq.app.pinpai == "nineone") {
                ...
                hqq["hall_test"].load("test/prefab/tianqi", cc.Prefab, (err, prefab) => {
                    ...
                    // hqq.addNode(goldback, { Res:hqq["hall_test"], normal: "tianqi/img/refresh", callback: "onClickGetGold", script: this, x: 200, size: { width: 35, height: 35 } })
                    ...
                    // hqq.setBtn(this.btnhby, { path: hrpath + "dating_hby", aniname: "animation", loop: true, widget: { top: 36, right: 450 }, active: false, callback: "onClickHBY", script: this })
                    // hqq.setBtn(this.btnjfcj, { path: hrpath + "dating_jfcj", aniname: "animation", loop: true, widget: { top: 45, right: 350 }, callback: "onClickJFCJ", script: this })
                    hqq.setBtn(btnjchd, { path: brpath + "icon_jchd", aniname: "Animation1", loop: true, widget: { top: -5, right: 165, closeleft: true, closebottom: true } })
                    hqq.setBtn(btnqmdl, { path: brpath + "quanming", aniname: "dating_qmdl", loop: true, widget: { top: 5, right: 70, closeleft: true, closebottom: true }, callback: "onClickQMDL", script: this })
                    hqq.setBtn(this.btngonggao, { normal: blpath + "icon_gonggao", widget: { top: 15, right: 18, closeleft: true, closebottom: true }, callback: "onClickGG", script: this })

                    hqq.setBtn(this.btnkefu, { normal: blpath + "icon_chat2", widget: { left: 70, bottom: 30, closeright: true, closetop: true }, callback: "onClickKF", script: this })
                    hqq.setBtn(this.btnshouyi, { normal: blpath + "icon_duihuan", widget: { left: 245, bottom: 30, closeright: true, closetop: true }, callback: "onClickSY", script: this })
                    hqq.setBtn(btnmfjb, { normal: blpath + "btnmfjb", widget: { left: 445, bottom: 30, closeright: true, closetop: true }, callback: "onClickMFJB", script: this })
                    // ---------
                    this.noticepanel.active = true
                    ...
                    // hqq.addNode(background, { normal: "base/language/CN/img/vip_diamond", widget: { horizontalCenter: 135, bottom: 20 }, callback: "toogleManyCai", script: this });

                    this.UILoadFinished();
                })
            } else if (hqq.app.isB2BList() || hqq.isOtherGame) {
                hqq.hall_test.load("test/prefab/tetu", cc.Prefab, (err, prefab) => {
                    if (err) {
                        cc.log(err);
                        return;
                    }
                    ...
                    this.noticepanel.active = true
                    background.getChildByName("tetu").active = true
                    hqq.setWidget(subgameview, { left: 250 })
                    this.menuBtnListNode = cc.find("Canvas/hallback/tetu/menu/dating_menu_di")
                    this.btnMenuSelect = cc.find("Canvas/hallback/tetu/menu/dating_menu_di/dating_btn_select_di")
                    let btn_remen = this.menuBtnListNode.getChildByName("btn_remen")
                    hqq.setBtn(btn_remen, { Res: hqq["hall_test"], normal: hrpath + "tetu/tetu_remen2", pressed: hrpath + "tetu/tetu_remen1", custom: this.menuBtnInfoList[0], callback: "onClickMenuBtnb2b", script: this, interactable: false, transition: cc.Button.Transition.SPRITE })

                    let btn_touzhu = this.menuBtnListNode.getChildByName("btn_touzhu")
                    hqq.setBtn(btn_touzhu, { Res: hqq["hall_test"], normal: hrpath + "tetu/ciji2", pressed: hrpath + "tetu/ciji1", custom: this.menuBtnInfoList[1], callback: "onClickMenuBtnb2b", script: this, transition: cc.Button.Transition.SPRITE })
                    let btn_duizhan = this.menuBtnListNode.getChildByName("btn_duizhan")
                    hqq.setBtn(btn_duizhan, { Res: hqq["hall_test"], normal: hrpath + "tetu/qipai2", pressed: hrpath + "tetu/qipai1", custom: this.menuBtnInfoList[2], callback: "onClickMenuBtnb2b", script: this, transition: cc.Button.Transition.SPRITE })

                    let btn_jieji = this.menuBtnListNode.getChildByName("btn_jieji")
                    hqq.setBtn(btn_jieji, { Res: hqq["hall_test"], normal: hrpath + "tetu/dianwan2", pressed: hrpath + "tetu/dianwan1", custom: this.menuBtnInfoList[3], callback: "onClickMenuBtnb2b", script: this, transition: cc.Button.Transition.SPRITE })
                    let pos = this.menuBtnListNode.children[0].getPosition();
                    this.btnMenuSelect.x = pos.x
                    this.btnMenuSelect.y = pos.y

                    cc.find("Canvas/btnmfjb").active = false;
                    btnjchd.active = false;
                    this.btngonggao.active = false;
                    btncopy.active = false;
                    ...
                    hqq.addNode(this.node, { Res: hqq["hall_test"], path: hrpath + "tetu/fjao", widget: { horizontalCenter: 0, bottom: 0 } });
                    hqq.addNode(this.node, { Res: hqq["hall_test"], normal: "tetu/img/fanhui", widget: { right: 20, top: 10 }, callback: "tooglePrevious", script: this });
                    this.UILoadFinished();
                })
            } else {
                hqq["hall_test"].load("test/prefab/yuyu", cc.Prefab, (err, prefab) => {
    ...
    UILoadFinished() {
        if (hqq.isOtherGame) {
            if (!hqq.open_im) {
                this.btnkefu.active = false;
            }
            if (!hqq.open_chongzhi) {
                cc.find("Canvas/btnchongzhi").active = false;
            }
            if (!hqq.open_tixian) {
                this.btnshouyi.active = false;
            }
            if (!hqq.open_proxy) {
                cc.find("Canvas/btnqmdl").active = false;
            }
        }

        this.suggameScrollView.content.parent.getComponent(cc.Widget).updateAlignment()

        if (hqq.app.pinpai != "ninetwo" && !hqq.isOtherGame) {
    ...
    onClickGetGold() {
        ...
        if (cc.isValid(btnplus) && hqq.app.pinpai != 'fuxin') {
            if (cc.TweenSystem.instance.ActionManager.getNumberOfRunningActionsInTarget(btnplus)) return;
            ...
            if (hqq.app.pinpai == "juding") {
                hqq.setSprite(btnplus, { path: "hall/juding/img/btn_refresh2" });
            }
        }
        this.getGold();
    }
    ...
    getGold() {
        let btnplus = cc.find("Canvas/goldback/btnplus");
        let callback = (msg) => {
            console.log("getGold callback msg", msg)
            if (msg.code !== 200) {
                ...
                if (cc.isValid(btnplus) && hqq.app.pinpai != 'fuxin') {
                    ...
                    cc.tween(btnplus)
                        ...
                        .call(() => {
                            ...
                            hqq.gameGlobal.player.gold = 0;
                            if (hqq.app.pinpai == "juding") {
                                hqq.setSprite(btnplus, { path: "hall/juding/img/btn_refresh" });
                            }
                        })
                        .start();
                }
                ...
                if (cc.isValid(btnplus) && hqq.app.pinpai != 'fuxin') {
                    ...
                    cc.tween(btnplus)
                        ...
                        .call(() => {
                            if (cc.isValid(this.coinlabel)) {
                                this.coinlabel.getComponent(cc.Label).string = gold.toString();
                            }
        ...
        let failcallback = (msg) => {
            console.log("getGold failcallback msg", msg)
            if (cc.isValid(btnplus) && hqq.app.pinpai != 'fuxin') {
                btnplus.getComponent(cc.Button).interactable = true;
                cc.Tween.stopAllByTarget(btnplus);
                cc.tween(btnplus)
    ...
    // 新贵游戏分类点击事件
    onClickXinguiMenuBtn(event: any, customEventData: any) {
    ...
    UISetJFCJ() {
        if (hqq.app.pinpai == "fuxin") {
            hqq.setBtn(this.btnjfcj, { Res: hqq["hall_test"], normal: "language/" + hqq.language + "/fuxin/icon4", widget: { top: 28, right: 390 }, callback: "onClickJFCJ", script: this })
        } else if (hqq.app.pinpai == "xingui") {
            hqq.setBtn(this.btnjfcj, { Res: hqq["hall_test"], normal: "language/" + hqq.language + "/xingui/btn_jfcj", widget: { top: 11, right: 510 }, callback: "onClickJFCJ", script: this })
        } else if (hqq.app.pinpai == "xinsheng") {
            hqq.setBtn(this.btnjfcj, { Res: hqq["hall_test"], normal: "language/" + hqq.language + "/xinsheng/btnjfcj", widget: { top: 15, right: 355 }, callback: "onClickJFCJ", script: this })
        } else if (hqq.app.pinpai == "xinhao") {
            ...
            hqq.setBtn(this.btnjfcj, { Res: hqq["hall_test"], normal: "language/" + hqq.language + "/icon_jfcj", widget: { top: 15, right: 510 }, callback: "onClickJFCJ", script: this })
        } else {
            hqq.setBtn(this.btnjfcj, { Res: hqq["hall_test"], path: "language/" + hqq.language + "/dating_jfcj", aniname: "animation", loop: true, widget: { top: 45, right: 350 }, callback: "onClickJFCJ", script: this })
        }
    }

    // 最先注册，及时监听来自子游戏的退出事件
    startInit() {
        hqq.eventMgr.register(hqq.eventMgr.hotCheckup, this.getClassName(), this.isupdataCallback.bind(this))
        hqq.eventMgr.register(hqq.eventMgr.hotFail, this.getClassName(), this.failCallback.bind(this))
        hqq.eventMgr.register(hqq.eventMgr.hotProgress, this.getClassName(), this.progressCallback.bind(this))
        hqq.eventMgr.register(hqq.eventMgr.hotFinish, this.getClassName(), this.finishCallback.bind(this))
        hqq.eventMgr.register(hqq.eventMgr.hotUp, this.getClassName(), this.setSubGameBtnUp.bind(this))
        hqq.eventMgr.register(hqq.eventMgr.hotCheck, this.getClassName(), this.setSubGameBtnCheck.bind(this))
        hqq.eventMgr.register(hqq.eventMgr.hotWait, this.getClassName(), this.setSubGameBtnUpWait.bind(this))
        hqq.eventMgr.register(hqq.eventMgr.refreshPlayerinfo, this.getClassName(), this.setPlayerInfo.bind(this))
        hqq.eventMgr.register(hqq.eventMgr.onReceiveBroadcast, this.getClassName(), this.onReceiveBroadcast.bind(this))
        hqq.eventMgr.register(hqq.eventMgr.refreshPlayerGold, this.getClassName(), this.onClickGetGold.bind(this))
        this.scheduleOnce(() => {
            cc.log("start init");
            this.setPlayerInfo();
            this.checkSubModule();
    ...
    checkSubModule() {
        ...
        this.scheduleOnce(() => {
            cc.log("checksubmodule");
            this.initWebSoketAndHttp();
        }, 0)
    }
    // 是否热更新返回
    isupdataCallback(bool: boolean, enname: string, isfail: boolean = false) {
        if (bool) { // 需要更新 自动更新，无需处理
            ...
            if (enname == "zrsx") {
                ...
                this.setSubGameBtnState("jdb", gameBtnState.isWait)
                this.setSubGameBtnState("wlby", gameBtnState.isWait)
                this.setSubGameBtnState("csby", gameBtnState.isWait)
            } else {
    ...
    progressCallback(progress: any, enname: any) {
        ...
        if (enname == "zrsx") {
            ...
            this.setSubGameBtnState("jdb", gameBtnState.progress)
            this.setSubGameBtnState("wlby", gameBtnState.progress)
            this.setSubGameBtnState("csby", gameBtnState.progress)
    ...
    finishCallback(enname: any) {
        ...
        if (enname == "zrsx") {
            ...
            this.setSubGameBtnState("jdb", gameBtnState.canClick)
            this.setSubGameBtnState("wlby", gameBtnState.canClick)
            this.setSubGameBtnState("csby", gameBtnState.canClick)
    ...
    setSubGameBtnUp(enname: any) {
        if (enname == "zrsx") {
            ...
            this.setSubGameBtnState("jdb", gameBtnState.needDown)
            this.setSubGameBtnState("wlby", gameBtnState.needDown)
            this.setSubGameBtnState("csby", gameBtnState.needDown)
        } else {
            if (hqq.app.pinpai == "xinhao" || hqq.app.pinpai == "xinsheng") {
                if (hqq.game1to2[enname]) {
                    enname = hqq.game1to2[enname];
    ...
    // 设置玩家信息
    setPlayerInfo(msg: any = null) {
        if (msg) {
            if (msg.game_nick) {
                this.namelabel.getComponent(cc.Label).string = msg.game_nick;
            }
            if (msg.id&&hqq.isOtherGame) {
                this.idlabel.getComponent(cc.Label).string = msg.id;
            }
            ...
            if (msg.game_img) {
                if (hqq.app.pinpai == "fuxin") {
                    ...
                    hqq.commonTools.loadHeadRes(msg.game_img, this.head.getComponent(cc.Sprite), 76);
                } else if (hqq.app.pinpai == "xinlong") {
                    hqq.commonTools.loadHeadRes(msg.game_img, this.head.getComponent(cc.Sprite), 80);
                } else {
            ...
            if (msg.phone_number || hqq.gameGlobal.player.phonenum || hqq.isOtherGame) {
                if (cc.isValid(this.bindbtn)) {
                    this.bindbtn.active = false;
                    this.youkeicon.active = false
                }
            ...
            if (hqq.isOtherGame) {
                this.idlabel.getComponent(cc.Label).string = player.id;
            }
            ...
            if (hqq.app.pinpai == "fuxin") {
                ...
                hqq.commonTools.loadHeadRes(player.headurl, this.head.getComponent(cc.Sprite), 76);
            } else if (hqq.app.pinpai == "xinlong") {
                hqq.commonTools.loadHeadRes(player.headurl, this.head.getComponent(cc.Sprite), 80);
            } else {
                hqq.commonTools.loadHeadRes(player.headurl, this.head.getComponent(cc.Sprite));
            }
            if (hqq.gameGlobal.player.phonenum != "" || hqq.isOtherGame) {
                this.youkeicon.active = false
                if (cc.isValid(this.bindbtn)) {
                    this.bindbtn.active = false;
                }
    ...
    initWebSoketAndHttp() {
        ...
        this.scheduleOnce(() => {
            cc.log("initsubgamebtn")
            this.initSubGameBtn();
        }, 0)
    }
    // 加载子游戏的子包 包含静态子包和动态子包
    loadBundle(subname: any,callback:Function = null) {
        // if (CC_DEBUG) {
        //     return
        // }
        // if (subname != "ddz") {
        ...
        if (subGamePinPaiResList[hqq.app.pinpai]) {
            ...
            for (key2 in subGamePinPaiResList[hqq.app.pinpai]) {
                if (key2 == subname) {
                    cc.assetManager.loadBundle(subname + "_" + hqq.app.pinpai, (err) => {
                        if (err) {
                            return cc.log('load subpackage script fail.', subname + "_" + hqq.app.pinpai);
                        }
                        hqq[subname + "_" + hqq.app.pinpai] = cc.assetManager.getBundle(subname + "_" + hqq.app.pinpai);
    ...
    */
    ...
    // 加载大厅子游戏按钮UI资源
    loadIconRes(node: any, name: any, key: any, i: any) {
        if (hqq.app.pinpai == "fuxin") {
            ...
            for (let key2 in subGameList.subGameList_test) {
                if (key2 === key && key2 != "yunyou") {
                    let path = "language/" + hqq.language + "/gameicon/" + this.subGameList[key].enname
                    // let path2 = "language/" + hqq.language + "/ninetwo/bigicon/icon" + this.subGameList[key].enname
                    hqq.setSprite(node, { Res: hqq["hall_" + hqq.app.pinpai], normal: path, width: 290, height: 270 ,scale:0.8})
                    return;
                }
            }
    ...
    onscrollEvents(scrollview, eventType, customEventData) {
        ...
        let tempitembtnwidth = this.itembtn.getComponent(cc.UITransform).width / 2.1;
        let tempx = Math.abs(pos.x - (this.scrollviewcotentX - tempitembtnwidth));
        let tempx2 = tempx + this.suggameScrollView.node.getChildByName("view").getComponent(cc.UITransform).width + (this.itembtn.getComponent(cc.UITransform).width);
        ...
        if (hqq.app.pinpai == "fuxin") {
            ...
            for (let key in this.subGameList) {
                ...
                if (cc.isValid(ani)) {
                    let type = null;
                    let tempcomparelist = [];
                    if (this.selectType === "remen") {
                        tempcomparelist = ["cylhd", "cdx", "lhd", "ebg", "caishendao", "duofuduocai", "fkxw", "hwby", "qznn", "ddz"];
                    } else if (this.selectType === "duizhan") {
                        tempcomparelist = ["ddz", "qznn", "sss", "zjh", "dzpk", "pdk", "21d", "ermj", "suoha"];
                    } else if (this.selectType === "jieji") {
                        tempcomparelist = ["bcbm", "hbld", "hwby", "jbpby", "sgj", "szwg", "cbzb", "duofuduocai", "fkxw", "caishendao", "fctbj"];
                    } else if (this.selectType === "touzhu") {
                        tempcomparelist = ["bjl", "brnn", "ebg", "hh", "lhd", "lp", "shaibao", "cylhd", "cdx", "szffc"];
                    }
                    // cc.log("key=",key," pos.x=",pos.x, " ani.x=",ani.x," tempx=",tempx," tempx2=",tempx2, " this.selectType=",this.selectType," tempcomparelist.indexOf(key)=",tempcomparelist.indexOf(key))
                    if ((ani.x < tempx || ani.x > tempx2)) {
                        if (ani.active) {
                            if ((pos.x > this.scrollviewcotentX && (ani.x == 158.5 || ani.x == 460.5 || ani.x == 762.5 || ani.x == 1064.5 || ani.x == 4990.5))) {
                                ani.active = true;
                            } else {
                                ani.active = false;
                            }
                        }
                    } else if (tempcomparelist.indexOf(key) != -1) {
                        ani.active = true;
                    }
                }
            };
    ...
    /** 子游戏初始化 */
    initSubGameBtn() {
        let width = (Math.ceil(Object.getOwnPropertyNames(this.subGameList).length / 2) * (this.itembtn.getComponent(cc.UITransform).width + 15)) + 30;
        ...
        this.suggameScrollView.getComponent(cc.ScrollView).content.getComponent(cc.UITransform).width = width
        this.suggameScrollView.getComponent(cc.ScrollView).content.setPositionEx(cc.v2(-this.suggameScrollView.node.getComponent(cc.UITransform).width / 2, 0))
        ...
        if (hqq.app.pinpai == "fuxin" || hqq.app.pinpai == "juding") {
            ...
            sortArray = ["cylhd", "cdx", "lhd", "ebg", "caishendao", "duofuduocai", "fkxw", "hwby", "qznn", "ddz"];
            for (let i = 0; i < Object.getOwnPropertyNames(this.subGameList).length; i++) {
                for (let key in this.subGameList) {
                    if (sortArray.indexOf(key) == -1) {
                        sortArray.push(key);
                        break;
                    }
                }
        ...
        for (let i = 0; i < sortArray.length; i++) {
            ...
            this.scheduleOnce(() => {
                ...
                if (hqq.app.pinpai == "xinsheng" || hqq.app.pinpai == "xingui") {
                    ...
                    hqq.setNode(itembtn.jiantou, { opacity: 125 });
                    hqq.setNode(itembtn.downflag, { x: 0, y: 0, width: 235, height: 235, opacity: 200 })
                    hqq.setNode(itembtn.progress, { opacity: 100 })
                    hqq.setSprite(itemhot, { path: "base/language/" + hqq.language + "/img/hot2", position: { x: -60, y: 57 } })
                    hqq.setSprite(itemwait, { path: "base/language/" + hqq.language + "/img/wait2", position: { x: 50, y: 57 } })
                    ...
                    // itembtn.scale = 1.2
                    // hqq.setNode(itembtn.progress, { opacity: 100 })
                    hqq.setSprite(itemhot, { Res: hqq["hall_" + hqq.app.pinpai], path: "language/" + hqq.language + "/xinlong/huo", position: { x: -55, y: 57 } })
                    hqq.setSprite(itemwait, { Res: hqq["hall_" + hqq.app.pinpai], path: "language/" + hqq.language + "/xinlong/jing", position: { x: -15, y: 57 } })
                    hqq.setSprite(itemexp, { Res: hqq["hall_" + hqq.app.pinpai], path: "language/" + hqq.language + "/xinlong/ti", position: { x: -30, y: 57 } })
                    itembtn.progress.active = false;
                    itembtn.progress = itembtn.getChildByName("progress2");
                    hqq.setSprite(itembtn.progress, { Res: hqq["hall_" + hqq.app.pinpai], path: "xinlong/img/xz2" });
                    hqq.setSprite(itembtn.progress.getChildByName("tuoyuan"), { Res: hqq["hall_" + hqq.app.pinpai], path: "xinlong/img/xz2" });
                    hqq.setSprite(itembtn.jiantou, { Res: hqq["hall_" + hqq.app.pinpai], path: "xinlong/img/xz1", width: 35, height: 47 });
                    itembtn.progresslabel.fontSize = 25;
                    itembtn.progresslabel.lineHeight = 25;
                    this.setSubBtnPos(itembtn, tempdata.hallid)
                } else {
                    hqq.setNode(itembtn.downflag, { x: -3, y: 3, width: 280, height: 210 })
                    ...
                    this.setSubBtnPos(itembtn, tempdata.hallid)
                    if (hqq.app.isB2BList() || hqq.isOtherGame) {
                        if (["cylhd", "cdx", "lhd", "ebg", "caishendao", "duofuduocai", "fkxw", "hwby", "qznn", "ddz"].indexOf(key) == -1) {
                            itembtn.active = false;
                        }
                    }
                }
                // this.setSubBtnPos(itembtn, tempdata.hallid);
                this.suggameScrollView.content.addChildEx(itembtn);
                ...
                if ((key == "hbld" && (hqq.app.pinpai != "huaxing" && hqq.app.pinpai != "ninetwo" && hqq.app.pinpai != "xinlong")) || key == "brnn" || key == 'jbpby' ||
        ...
        if (!cc.isValid(this.node)) return;
        let scrollEventHandler = new cc.EventHandler();
        scrollEventHandler.target = this.node;
        scrollEventHandler.component = this.getClassName();
        ...
        this.scheduleOnce(() => {
            ...
            let width = (Math.ceil(btnnum / 2) * (this.itembtn.getComponent(cc.UITransform).width + 15)) + 30;
            ...
            this.suggameScrollView.content.getComponent(cc.UITransform).width = width;
            this.suggameScrollView.content.x = -this.suggameScrollView.node.getComponent(cc.UITransform).width / 2
            if (    hqq.app.isB2BList()){
                this.refreshSubGameBtnb2b("remen");
            }
            this.onscrollEvents(this.suggameScrollView, null, null);
    ...
    checkSubGameDownload(enname: any) {
        let subdata = this.getRemoteSubgame(this.subGameList[enname].game_id)
        if (subdata && subdata.open == 0) {
            ...
            if (needup && !cc.sys.isBrowser && enname != "aga") { // && cc.sys.os != "Windows"
                ...
                this.setSubGameBtnState(enname, gameBtnState.canClick)
                if (hqq.app.isRelease && !cc.sys.isBrowser) {
    ...
    // 聚鼎左侧菜单按钮统一回调
    onClickJudingMenuBtn(event: any, customEventData: any) {
        for (let i = 1; i < this.menuBtnListNode.children.length; i++) {
            if (this.menuBtnListNode.children[i].name.match(customEventData)) {
                this.menuBtnListNode.children[i].getComponent(cc.Button).interactable = false
                if (this.btnMenuSelect) {
    ...
    // 点击左侧菜单按钮统一回调
    onClickMenuBtn(event: any, customEventData: any) {
        console.log("onClickMenuBtn")
        for (let i = 0; i < this.menuBtnListNode.children.length; i++) {
            if (this.menuBtnListNode.children[i].getComponent(cc.Button)) {
                if (this.menuBtnListNode.children[i].name.match(customEventData)) {
                    this.menuBtnListNode.children[i].getComponent(cc.Button).interactable = false
                    if (this.btnMenuSelect) {
                        let pos = this.menuBtnListNode.children[i].getPosition().add(this.menuBtnListNode.getPosition())
    ...
    // 点击渔鱼菜单按钮统一回调
    onClickYuyuMenuBtn(event: any, customEventData: any) {
        console.log("onClickYuyuMenuBtn")
        for (let i = 0; i < this.menuBtnListNode.children.length; i++) {
            if (this.menuBtnListNode.children[i].name.match(customEventData)) {
                this.menuBtnListNode.children[i].getComponent(cc.Button).interactable = false
                this.refreshSubGameBtn(customEventData)
    ...
    // 点击左侧菜单按钮统一回调b2b
    onClickMenuBtnb2b(event: any, customEventData: any) {
        console.log("onClickMenuBtnb2b customEventData=", customEventData)
        for (let i = 0; i < this.menuBtnListNode.children.length; i++) {
            if (this.menuBtnListNode.children[i].getComponent(cc.Button)) {
                if (this.menuBtnListNode.children[i].name.match(customEventData)) {
                    this.menuBtnListNode.children[i].getComponent(cc.Button).interactable = false
                    ...
                    this.refreshSubGameBtnb2b(customEventData)
                } else {
                    this.menuBtnListNode.children[i].getComponent(cc.Button).interactable = true
                }
            }
    ...
    /** 点击子游戏按钮统一回调 */
    onClickSubgame(event: any, enname: any) {
    ...
    jumpToSubGame(enname: any) {
        ...
        if (enname == "hbsl" || enname == 'zrsx1' || enname == 'zrsx2'
            ...
            if(cc.sys.platform === cc.sys.Platform.ANDROID || cc.sys.os === cc.sys.OS.ANDROID
                || cc.sys.platform === cc.sys.Platform.IOS || cc.sys.platform === cc.sys.Platform.MACOS || cc.sys.os === cc.sys.OS.IOS)
            {
        ...
        if(cc.sys.isBrowser){
            this.loadBundle(key,()=>{this.loadNextScene(enname)});
        } else{
    ...
    loadNextScene(enname: any) {
        if (hqq.app.pinpai == "fuxin") {
            cc.director.preloadScene(this.subGameList[enname].fuxin_lanchscene, this.preloadSceneOnProgress, (err: Error) => {
                if (err) {
                    cc.log(err)
            ...
            cc.director.preloadScene(this.subGameList[enname].lanchscene, this.preloadSceneOnProgress, (err: Error) => {
                if (err) {
                    cc.log(err)
                    hqq.logMgr.logerror(err)
                    return
    ...
    // 金币数字后的加号按钮
    onClickPlus(event: any, customEventData: any) {
    ...
    // 充值
    onClickCC(event: any, customEventData: any) {
    ...
    // 免费金币
    onClickMFJB(event: any, customEventData: any) {
        console.log("onClickMFJB")
        this.clickDelay(event)
        if (hqq.app.pinpai == "juding") {
    ...
    // 聊天客服
    onClickKF(event: any, customEventData: any) {
        cc.log("onClickKF")
        this.clickDelay(event)
    ...
    // 收益 兑换提现
    onClickSY(event: any, customEventData: any) {
        cc.log("onClickSY")
        this.clickDelay(event)
        if (hqq.app.pinpai == "yuyu") {
            hqq.eventMgr.dispatch(hqq.eventMgr.showTip, hqq.getTip("str8"))
        ...
        if (hqq.subModel.cash.lanchscene != "") {
            hqq.eventMgr.dispatch(hqq.eventMgr.showJumpScene, "cash");
        } else {
            cc.log("请配置提现场景")
        }
    }
    ...
    // 红包雨
    onClickHBY(event: any, customEventData: any) {
        console.log("onClickHBY")
        if (hqq.HBYTick.isGet) {
    ...
    showHBYAni() {
        ...
        let callback = (gold) => {
            ...
            ani2.getComponent(cc.sp.Skeleton).setAnimation(0, 'hb_open', true)
        }
    ...
    // 积分抽奖
    onClickJFCJ(event: any, customEventData: any) {
        console.log("onClickJFCJ")
        this.clickDelay(event)
        hqq.eventMgr.dispatch(hqq.eventMgr.showPayActivityWeb, true);
    }
    // 精彩活动
    onClickJCHD(event: any, customEventData: any) {
        console.log("onClickJCHD")
        this.clickDelay(event)
        this.jchdRedPoint && (this.jchdRedPoint.active = false, hqq.hallactivitybtn = true);
        hqq.eventMgr.dispatch(hqq.eventMgr.showPayActivityWeb, true);
    }
    // 全名代理
    onClickQMDL(event: any, customEventData: any) {
        cc.log("onClickQMDL")
        this.clickDelay(event)
        if (hqq.app.pinpai == "yuyu") {
            hqq.eventMgr.dispatch(hqq.eventMgr.showTip, hqq.getTip("str8"))
    ...
    // 公告
    onClickGG(event: any, customEventData: any) {
        cc.log("onClickGG")
        ...
        let mycallback = () => {
            ...
            if (mainversion != hqq.app.subGameVersion.hall) {
                ...
                if (upstate) {
                    ...
                    this.setSubGameBtnState(subgamern, gameBtnState.needDown)
                }
            }
        }
    ...
    // 设置子游戏按钮位置(2列)
    setSubBtnPos(itembtn: hqqNode, index: any) {
        if (!cc.isValid(itembtn)) return;
        itembtn.active = true
        itembtn.x = Math.floor(index / 2) * (this.itembtn.getComponent(cc.UITransform).width + 15) + this.itembtn.getComponent(cc.UITransform).width / 2 + 15;
        if (hqq.app.pinpai == "fuxin") {
            itembtn.y = -index % 2 * (this.itembtn.getComponent(cc.UITransform).height + 27) + this.itembtn.getComponent(cc.UITransform).height / 2 + 8;
        } else if (hqq.app.pinpai == "juding") {
            if (index == 0 || index == 1) {
                itembtn.x = this.itembtn.getComponent(cc.UITransform).width / 2 + Math.floor(index / 2) * (this.itembtn.getComponent(cc.UITransform).width);
            } else {
                itembtn.x = this.itembtn.getComponent(cc.UITransform).width / 2 + Math.floor(index / 2) * (this.itembtn.getComponent(cc.UITransform).width / 1.4);
            }
            itembtn.y = -index % 2 * (this.itembtn.getComponent(cc.UITransform).height) + this.itembtn.getComponent(cc.UITransform).height / 2;
        } else if (hqq.app.pinpai == "xinsheng" || hqq.app.pinpai == "xingui" || hqq.app.pinpai == "xinlong") {
            itembtn.y = -index % 2 * (this.itembtn.getComponent(cc.UITransform).height + 50) + this.itembtn.getComponent(cc.UITransform).height / 2 + 8;
        } else if (hqq.app.pinpai == "huaxing") {
            if (index == 0 || index == 1) {
                itembtn.x = this.itembtn.getComponent(cc.UITransform).width / 2 + Math.floor(index / 2) * (this.itembtn.getComponent(cc.UITransform).width);
            } else {
                itembtn.x = this.itembtn.getComponent(cc.UITransform).width / 2 + Math.floor(index / 2) * (this.itembtn.getComponent(cc.UITransform).width / 1.1);
            }
            itembtn.y = -index % 2 * (this.itembtn.getComponent(cc.UITransform).height) + this.itembtn.getComponent(cc.UITransform).height / 2 + 8;
        } else if (hqq.app.pinpai == "ninetwo") {
            itembtn.x = this.itembtn.getComponent(cc.UITransform).width / 2 + Math.floor(index / 2) * (this.itembtn.getComponent(cc.UITransform).width / 1.25);
            if (index % 2 == 0) {
                itembtn.y = this.itembtn.getComponent(cc.UITransform).height / 2;
            } else {
                itembtn.y = -(this.itembtn.getComponent(cc.UITransform).height + 15) + this.itembtn.getComponent(cc.UITransform).height / 2;
            }
            hqq.setNode(itembtn.getChildByName("wait"), { opacity: 255, x: -15, y: 57 });
        } else {
            itembtn.y = -index % 2 * (this.itembtn.getComponent(cc.UITransform).height) + this.itembtn.getComponent(cc.UITransform).height / 2;
        }
    }
    ...
    setSubBtnPosOne(itembtn, index) {
        ...
        if (hqq.app.pinpai == "ninetwo") {
            ...
            hqq.setNode(itembtn.getChildByName("wait"), { opacity: 255 })
        }
    }
    // 获取子游戏登陆历史
    getSubGameLoginHistory(enname: any): Array<number> {
    ...
    // 刷新子游戏按钮
    refreshSubGameBtn(customEventData: any) {
        ...
        if (customEventData == 'all') {
            ...
            for (let i = 0; i < this.subGameBtnArr.length; i++) {
                ...
                // cc.log("refreshSubGameBtn type=", type, " key=", key, " this.subGameList[key].gameType=", this.subGameList[key].gameType, " i=", i)
    ...
    // 刷新子游戏按钮
    refreshSubGameBtnb2b(customEventData: any) {
        this.suggameScrollView.scrollToLeft(0.5)
        let btnnum = 0
        this.selectType = customEventData;
        ...
        if (customEventData === "remen") {
            ...
            tempcomparelist = ["ddz", "qznn", "sss", "zjh", "dzpk", "pdk", "21d", "ermj", "suoha"];
        } else if (customEventData === "jieji") {
            tempcomparelist = ["bcbm", "hbld", "hwby", "jbpby", "sgj", "szwg", "cbzb", "duofuduocai", "fkxw", "caishendao", "fctbj"];
        } else if (customEventData === "touzhu") {
            tempcomparelist = ["bjl", "brnn", "ebg", "hh", "lhd", "lp", "shaibao", "cylhd", "cdx", "szffc"];
        }
        let i = 0;
        let j = 0;
        ...
        cc.log("refreshSubGameBtnb2b customEventData=", customEventData, " btnnum=", btnnum)
        let width = (Math.ceil(btnnum / 2) * (this.itembtn.getComponent(cc.UITransform).width + 15)) + 30;
    ...
    // 刷新富鑫子游戏按钮
    refreshFuxinSubGameBtn(customEventData: any) {
    ...
    // 刷新聚鼎子游戏按钮
    refreshJudingSubGameBtn(customEventData: any) {
    ...
    getSgjPool() {
        let failcallback = () => {
            this.unschedule(this.getSgjPool)
        }
            ...
            if (this.subGameBtnMap["sgj"].getChildByName("goldlabel")) {
                ...
                if (/* hqq.app.pinpai == "xinsheng" || */ hqq.app.pinpai == "xingui" || hqq.app.pinpai == "xinlong") {
                    ...
                    if (hqq.app.pinpai == "xinlong") {
                        ...
                        this.subGameBtnMap["sgj"].getChildByName("ani").zIndex = 0;
                        this.subGameBtnMap["sgj"].getChildByName("nameimg").zIndex = 1;
                        this.subGameBtnMap["sgj"].getChildByName("downFlag2").zIndex = 2;
                        this.subGameBtnMap["sgj"].getChildByName("downFlag").zIndex = 3;
                        this.subGameBtnMap["sgj"].getChildByName("progresslabel").zIndex = 4;
                        this.subGameBtnMap["sgj"].getChildByName("wait").zIndex = 5;
                        this.subGameBtnMap["sgj"].getChildByName("experience").zIndex = 6;
                        this.subGameBtnMap["sgj"].getChildByName("hot").zIndex = 7;
                        node0.zIndex = 8;
                        tempzindex = 9;
                        this.subGameBtnMap["sgj"].getChildByName("progress").zIndex = 10;
                        this.subGameBtnMap["sgj"].getChildByName("progress2").zIndex = 11;
                        this.subGameBtnMap["sgj"].getChildByName("jiantou").zIndex = 12;
                    } else {
                        this.subGameBtnMap["sgj"].addChildEx(node0, this.subGameBtnMap["sgj"].getChildByName("ani").zIndex)
                    }
                } else if (hqq.app.pinpai == "huaxing") {
                    hqq.addNode(this.subGameBtnMap["sgj"].getChildByName("ani"), { Res: hqq["hall_" + hqq.app.pinpai], path: "huaxing/img/jjck", x: 0, y: 70, zIndex: 0 });
                    ...
                    hqq.addNode(this.subGameBtnMap["sgj"].getChildByName("ani"), { Res: hqq["hall_" + hqq.app.pinpai], path: "ninetwo/img/jjck", x: 3, y: 70, zIndex: 0, width: 170, type: cc.Sprite.Type.SLICED });
                    if (!tempbool) {
    ...
    getHbslPool() {
        let failcallback = () => {
            this.unschedule(this.getHbslPool)
        }
            ...
            if (this.subGameBtnMap["hbsl"].getChildByName("goldlabel")) {
                ...
                if (/* hqq.app.pinpai == "xinsheng" || */ hqq.app.pinpai == "xingui" || hqq.app.pinpai == "xinlong") {
                    ...
                    if (hqq.app.pinpai == "xinlong") {
                        ...
                        this.subGameBtnMap["hbsl"].getChildByName("ani").zIndex = 0;
                        this.subGameBtnMap["hbsl"].getChildByName("nameimg").zIndex = 1;
                        this.subGameBtnMap["hbsl"].getChildByName("downFlag2").zIndex = 2;
                        this.subGameBtnMap["hbsl"].getChildByName("downFlag").zIndex = 3;
                        this.subGameBtnMap["hbsl"].getChildByName("progresslabel").zIndex = 4;
                        this.subGameBtnMap["hbsl"].getChildByName("wait").zIndex = 5;
                        this.subGameBtnMap["hbsl"].getChildByName("experience").zIndex = 6;
                        this.subGameBtnMap["hbsl"].getChildByName("hot").zIndex = 7;
                        node0.zIndex = 8;
                        tempzindex = 9;
                        this.subGameBtnMap["hbsl"].getChildByName("progress").zIndex = 10;
                        this.subGameBtnMap["hbsl"].getChildByName("progress2").zIndex = 11;
                        this.subGameBtnMap["hbsl"].getChildByName("jiantou").zIndex = 12;
                    } else {
                        this.subGameBtnMap["hbsl"].addChildEx(node0, this.subGameBtnMap["hbsl"].getChildByName("ani").zIndex)
                    }
                } else if (hqq.app.pinpai == "huaxing") {
                    hqq.addNode(this.subGameBtnMap["hbsl"].getChildByName("ani"), { Res: hqq["hall_" + hqq.app.pinpai], path: "huaxing/img/jjck", x: 0, y: 70, zIndex: 0 });
                    node.setPositionEx(0, 68)
                } else if (hqq.app.pinpai == "ninetwo") {
                    label.fontSize = 22
                    ...
                    hqq.addNode(this.subGameBtnMap["hbsl"].getChildByName("ani"), { Res: hqq["hall_" + hqq.app.pinpai], path: "ninetwo/img/jjck", x: 3, y: 70, zIndex: 0, width: 170, type: cc.Sprite.Type.SLICED });
                    if (!tempbool) {
    ...
    getHbldPool() {
        let failcallback = () => {
            this.unschedule(this.getHbldPool)
        }
            ...
            if (this.subGameBtnMap["hbld"].getChildByName("goldlabel")) {
                ...
                if (/* hqq.app.pinpai == "xinsheng" || */ hqq.app.pinpai == "xingui" || hqq.app.pinpai == "xinlong") {
                    ...
                    if (hqq.app.pinpai == "xinlong") {
                        ...
                        this.subGameBtnMap["hbld"].getChildByName("ani").zIndex = 0;
                        this.subGameBtnMap["hbld"].getChildByName("nameimg").zIndex = 1;
                        this.subGameBtnMap["hbld"].getChildByName("downFlag2").zIndex = 2;
                        this.subGameBtnMap["hbld"].getChildByName("downFlag").zIndex = 3;
                        this.subGameBtnMap["hbld"].getChildByName("progresslabel").zIndex = 4;
                        this.subGameBtnMap["hbld"].getChildByName("wait").zIndex = 5;
                        this.subGameBtnMap["hbld"].getChildByName("experience").zIndex = 6;
                        this.subGameBtnMap["hbld"].getChildByName("hot").zIndex = 7;
                        node0.zIndex = 8;
                        tempzindex = 9;
                        this.subGameBtnMap["hbld"].getChildByName("progress").zIndex = 10;
                        this.subGameBtnMap["hbld"].getChildByName("progress2").zIndex = 11;
                        this.subGameBtnMap["hbld"].getChildByName("jiantou").zIndex = 12;
                    } else {
                        this.subGameBtnMap["hbld"].addChildEx(node0, this.subGameBtnMap["hbld"].getChildByName("ani").zIndex)
                    }
                } else if (hqq.app.pinpai == "huaxing") {
                    hqq.addNode(this.subGameBtnMap["hbld"].getChildByName("ani"), { Res: hqq["hall_" + hqq.app.pinpai], path: "huaxing/img/jjck", x: 0, y: 70, zIndex: 0 });
                    node.setPositionEx(0, 68)
                } else if (hqq.app.pinpai == "ninetwo") {
                    label.fontSize = 22
                    ...
                    hqq.addNode(this.subGameBtnMap["hbld"].getChildByName("ani"), { Res: hqq["hall_" + hqq.app.pinpai], path: "ninetwo/img/jjck", x: 3, y: 70, zIndex: 0, width: 170, type: cc.Sprite.Type.SLICED });
                    if (!tempbool) {
    ...
    getNotice() {
        if (hqq.gameGlobal.noticeList.length > 0) {
            ...
            if (!isallread) {
                ...
                hqq.eventMgr.register(hqq.eventMgr.refreshHallTips, this.getClassName(), this.setNoticeReadState.bind(this))
            }
            return
        }
        let callback = (data, url) => {
            ...
            if (data.code == 200) {
                if (!data.msg || data.msg.length == 0) {
                    ...
                    if (hqq.gameGlobal.noticeList.length > 0) {
                        ...
                        hqq.eventMgr.register(hqq.eventMgr.refreshHallTips, this.getClassName(), this.setNoticeReadState.bind(this))
                        if (hqq.needShowNotice && hqq.app.pinpai != "xinhao" && hqq.app.pinpai != "juding" && hqq.app.pinpai != "nineone" && hqq.app.pinpai != "huaxing" && hqq.app.pinpai != "xinsheng" && hqq.app.pinpai != "huangshi" && hqq.app.pinpai != "xinlong") {
    ...
    onDestroy() {
        ...
        this.unschedule(this.getHbslPool)
        this.unschedule(this.getHbldPool)
        this.unschedule(this.timeTickHBY)
        this.sgjxhr && this.sgjxhr.abort()
        this.hbslxhr && this.hbslxhr.abort()
        this.hbldxhr && this.hbldxhr.abort()
        hqq.eventMgr.unregister(hqq.eventMgr.hotCheckup, this.getClassName())
        hqq.eventMgr.unregister(hqq.eventMgr.hotFail, this.getClassName())
        hqq.eventMgr.unregister(hqq.eventMgr.hotProgress, this.getClassName())
        hqq.eventMgr.unregister(hqq.eventMgr.hotFinish, this.getClassName())
        hqq.eventMgr.unregister(hqq.eventMgr.hotUp, this.getClassName())
        hqq.eventMgr.unregister(hqq.eventMgr.hotCheck, this.getClassName())
        hqq.eventMgr.unregister(hqq.eventMgr.hotWait, this.getClassName())
        hqq.eventMgr.unregister(hqq.eventMgr.refreshPlayerinfo, this.getClassName())
        hqq.eventMgr.unregister(hqq.eventMgr.refreshHallTips, this.getClassName())
    }

    _creatSubAccount() {
        let mgameid = [
            "5b1f3a3cb76a451e220717",
        ]
        let sub = [
            801717971,
        ]
        if (typeof this.tempindex === undefined || this.tempindex === null ) {
            this.tempindex = 0;
        }
        if (typeof this.tempindex2 === undefined || this.tempindex2 === null ) {
            this.tempindex2 = 0;
        }
        ...
        hqq.hallWebSocket.unregister("/Game/Login/login", this.getClassName())
        hqq.hallWebSocket.close()
    ...
    resetSubGameList() {
        ...
        // 新增要的游戏
        if (subGameList) {
            for (let a in subGameList.subGameList_test) {
                hqq.subGameList[a] = subGameList.subGameList_test[a];
            }
        }
        hqq.app.checkSubServer();

        this.subGameList = hqq.commonTools.jsonCopy(hqq.subGameList);
        if (hqq.app.pinpai == "nineone" || hqq.app.pinpai == "huangshi" ||
            ...
            if (this.subGameList["sanshengtiyu"]) {
