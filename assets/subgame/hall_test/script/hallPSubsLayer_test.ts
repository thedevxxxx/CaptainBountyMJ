...
import hqqSlider from "../../../base/common/hqqSlider";

const { ccclass, property } = cc._decorator;

@ccclass("hallPSubsLayer_test")
export class hallPSubsLayer_test extends cc.Component {
    ...
    getClassName() {
        return "hallPSubsLayer_" + hqq.app.pinpai;
    }
    ...
    UILoad() {
        ...
        if (hqq.app.pinpai == "fuxin") {
            ...
            node.getComponent(cc.Label).color = cc.color(0, 0, 0);
    ...
    weblogin_click() {
        if (this.weblogin.getChildByName("pass1").active) {// 注册
            ...
            cc.log("weblogin_click=====================account=", account, " pass=", pass)
            hqq.loginMgr.accountChange(account, pass, (issucess) => {
                cc.log("weblogin_click=====================issucess=", issucess)
                if (issucess) {
    ...
    onSettingMenuClick(toggle, customEventData) {
        ...
        if (Number(customEventData) === 1) {
            ...
            cc.find("smallsublayer/setting/btncontainer/ylsz/checkmark/yjks/yylxradio/toggle" + musicindex).getComponent(cc.Toggle).isChecked;
    ...
    resetpassEnsure() {
        ...
        let failcallback = (status, forcejump, url, err, readyState) => {
            hqq.logMgr.log("resetpassEnsure failcallback", status, forcejump, url, err, readyState)
            if (cc.isValid(resetsurebtn)) {
                resetsurebtn.interactable = true
            }
            hqq.eventMgr.dispatch(hqq.eventMgr.showTip, hqq.getTip("requestfail") + status + ",err:" + err)
