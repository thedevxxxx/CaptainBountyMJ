...
import hqqWebview from "../../../base/common/hqqWebview";

@ccclass('hqqWebview_test')
export default class hqqWebview_test extends hqqWebview {
    getClassName() {
        return "hqqWebview_test";
    }
    onLoad() {
        this.web = cc.find("PayActivityWeb/web").getComponent(cc.WebView);
        if (cc.isValid(this.web)) {
            if( !cc.isValid(this.web.node.getComponent(cc.UIOpacity))){
                this.web.addComponent(cc.UIOpacity);
            }
            hqq.setNode(this.web.node,{opacity:0})
            this.web.node.getComponent(cc.UITransform).width = 0;
            this.web.node.getComponent(cc.UITransform).height = 0;
            let loadEventHandler = new cc.EventHandler();
            loadEventHandler.target = this.node;
            loadEventHandler.component = this.getClassName();
            loadEventHandler.handler = "onWebviewEvent";

            if (this.web.webviewEvents.length > 0) {
                this.web.webviewEvents[0] = loadEventHandler;
            } else {
                this.web.webviewEvents.push(loadEventHandler);
            }
        }
    }
    start() {
    ...
    public onWebviewEvent(webview: cc.WebView, eventType: cc.__private._cocos_web_view_web_view_enums__EventType): void {
        if (eventType === "loaded") {
            ...
            if (cc.isValid(this.web)) {
                ...
                hqq.setNode(this.web.node,{opacity:255})
                ...
                if (hqq.app.pinpai == "fuxin") {
                    ...
                    hqq.addNode(cc.find("main_top", this.node), { path: "base/huaxing/img/d_tit", width: 458, height: 87, x: 0, zIndex: 0, type: cc.Sprite.Type.SLICED });
