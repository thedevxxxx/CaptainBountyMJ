...
@ccclass('PayUsdtAccountAlert1')
export default class PayUsdtAccountAlert1 extends Component {
    @property(EditBox)
    walletAddressInput: EditBox | null = null;
    @property(Label)
    ...
    fetchBindAccountPay() {
        ...
        this.app.ajax('POST',url,dataStr,(response)=>{
            if(response.status == 0){
                let bankCom = cc.find('Canvas/Cash/Content/UsdtDh').getComponent('PayUsdtDh1');
                bankCom.fetchIndex();
