...
@ccclass('PayPublicOrderAlert1')
export default class PayPublicOrderAlert1 extends Component {
    @property(Node)
    popWindowBG : Node | null = null;
    @property(Prefab)
    helpAlert: Prefab | null = null;
    @property(Label)
    ...
    blinkFun(){
        let label1Node = cc.find("Canvas/PublicOrderAlert/Content/label1").getComponent(cc.Label)
        let label2Node = cc.find("Canvas/PublicOrderAlert/Content/label2").getComponent(cc.Label)
        let R = 255
        let G = 212
        let B = 105
        let endG = 0
