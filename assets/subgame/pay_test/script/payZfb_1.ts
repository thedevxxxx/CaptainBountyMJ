...
import { appGlobal } from "../../../base/app/appGlobal";
import { _decorator, Component, Label, Prefab, Node} from 'cc';
import * as cc from 'cc';
const {ccclass, property} = _decorator;

@ccclass('PayZfb1')
export default class PayZfb1 extends Component {
    @property(Label)
    huodongLabel: Label | null = null;
    @property(Prefab)
    SelectItem: Prefab | null =null;
    @property(Node)
    selectContent: Node | null =null;
    @property(Label)
    czArea: Label | null = null;
    @property(Node)
    icon: Node | null = null;
    @property(Node)
    iconFont: Node | null = null;
    @property(Label)
    wxtsLabel: Label | null = null;
    @property
    showSelect = false;
    @property(Label)
    amountLabel: Label | null = null;
    @property(Node)
    shuiyin: Node | null = null;
    @property(Node)
    blinkNode: Node | null = null;
    @property(Prefab)
    ...
    init(data){
        ...
        }else if(this.channel =='EBpay'){
            this.wxtsLabel.string = `1.充值比例1元=1金币`;
            this.app.loadPublicIcon(`recharge/flag_eb`,this.icon,100,100)
            this.app.loadIcon(`${src}/font/ebpay`,this.iconFont.children[0],200,45)
        }else if(this.channel =='TOpay'){
            this.wxtsLabel.string = `1.充值比例1元=1金币`;
            this.app.loadPublicIcon(`recharge/flag_to`,this.icon,100,100)
            this.app.loadIcon(`${src}/font/topay`,this.iconFont.children[0],200,45)
        }else if(this.channel =='OKpay'){
            this.wxtsLabel.string = `1.充值比例1元=1金币`;
            this.app.loadPublicIcon(`recharge/flag_ok`,this.icon,100,100)
            this.app.loadIcon(`${src}/font/okpay`,this.iconFont.children[0],200,45)
        }
    ...
    public fetchZfb(){
        ...
        this.app.ajax('GET',url,'',(response)=>{
            ...
            if(response.status == 0){
                ...
                }else if(self.channel =='EBpay'){
                    self.results = response.data.eb_pay;
                }else if(self.channel =='TOpay'){
                    self.results = response.data.to_pay;
                }else if(self.channel =='OKpay'){
                    self.results = response.data.ok_pay;
                }
    ...
    public initRender(index){
        ...
        if(this.current.name == this.handling_feeName && this.handling_fee !=0 ){
            ...
            if( this.handling_fee!=0){
                handling_feeLabel.getComponent(cc.Label).string = `手续费率${this.app.config.toDecimal(this.handling_fee*100)}%`
            ...
            let callBack = (is_first)=>{
                ...
                if(this.first_min <=0) {
                    ...
                    arr.forEach((e)=>{
                        ...
                        node.getComponent("PayBtnNum1").init(e,this.addGold.bind(this))
                    ...
                    if(this.app.UrlData.package_id  == 16){
                        ...
                        arr.forEach((e)=>{
                            ...
                            node.getComponent("PayCyjeItem1").init(e,this.addGold.bind(this))
    ...
    public onClick(){
        ...
        if(this.amountLabel.string ==Language_pay.Lg.ChangeByText('点击输入')){
            ...
            if(this.channel == 'bankcard_transfer'){
                ...
                //小额渠道提示
                let url = `${this.app.UrlData.host}/api/payment/payment?user_id=${this.app.UrlData.user_id}&user_name=${decodeURI(this.app.UrlData.user_name)}&payment_amount=${this.amountLabel.string}&channel_type=${this.current.channel_id}&channel_name=${this.current.name}&pay_name=${this.current.nick_name}&pay_type=${this.current.pay_type}&client=${this.app.UrlData.client}&proxy_user_id=${this.app.UrlData.proxy_user_id}&proxy_name=${decodeURI(this.app.UrlData.proxy_name)}&package_id=${this.app.UrlData.package_id}&order_ip=${this.login_ip ? this.login_ip:"127.0.0.1"}&device_id=${hqq.app.deviceID}&token=${this.app.token}&center_auth=${this.app.login_token}`
                this.app.showZfbWxAlert(url);
            }else if(this.channel== "EBpay"){
                let url = `${this.app.UrlData.host}/api/payment/ebpay/transfer?user_id=${this.app.UrlData.user_id}&user_name=${decodeURI(this.app.UrlData.user_name)}&amount=${this.amountLabel.string}&channel_type=${this.current.channel_id}&channel_name=${this.current.name}&pay_name=${this.current.nick_name}&pay_type=${this.current.pay_type}&client=${this.app.UrlData.client}&proxy_user_id=${this.app.UrlData.proxy_user_id}&proxy_name=${decodeURI(this.app.UrlData.proxy_name)}&package_id=${this.app.UrlData.package_id}&order_ip=${this.login_ip ? this.login_ip:"127.0.0.1"}&device_id=${hqq.app.deviceID}&token=${this.app.token}&center_auth=${this.app.login_token}`
                cc.sys.openURL(encodeURI(url))
                cc.log(encodeURI(url))
            }else if(this.channel== "TOpay"){
                let url = `${this.app.UrlData.host}/api/payment/topay/transfer?user_id=${this.app.UrlData.user_id}&user_name=${decodeURI(this.app.UrlData.user_name)}&amount=${this.amountLabel.string}&channel_type=${this.current.channel_id}&channel_name=${this.current.name}&pay_name=${this.current.nick_name}&pay_type=${this.current.pay_type}&client=${this.app.UrlData.client}&proxy_user_id=${this.app.UrlData.proxy_user_id}&proxy_name=${decodeURI(this.app.UrlData.proxy_name)}&package_id=${this.app.UrlData.package_id}&order_ip=${this.login_ip ? this.login_ip:"127.0.0.1"}&device_id=${hqq.app.deviceID}&token=${this.app.token}&center_auth=${this.app.login_token}`
                cc.sys.openURL(encodeURI(url))
                cc.log(encodeURI(url))
            }else if(this.channel== "OKpay"){
                let url = `${this.app.UrlData.host}/api/payment/okpay/transfer?user_id=${this.app.UrlData.user_id}&user_name=${decodeURI(this.app.UrlData.user_name)}&amount=${this.amountLabel.string}&channel_type=${this.current.channel_id}&channel_name=${this.current.name}&pay_name=${this.current.nick_name}&pay_type=${this.current.pay_type}&client=${this.app.UrlData.client}&proxy_user_id=${this.app.UrlData.proxy_user_id}&proxy_name=${decodeURI(this.app.UrlData.proxy_name)}&package_id=${this.app.UrlData.package_id}&order_ip=${this.login_ip ? this.login_ip:"127.0.0.1"}&device_id=${hqq.app.deviceID}&token=${this.app.token}&center_auth=${this.app.login_token}`
                cc.sys.openURL(encodeURI(url))
                cc.log(encodeURI(url))
            }
            else{
    ...
    showPayIM(){
        var url = `${this.app.UrlData.host}/api/payment/payment?user_id=${this.app.UrlData.user_id}&user_name=${decodeURI(this.app.UrlData.user_name)}&payment_amount=${this.amountLabel.string}&channel_type=${this.current.channel_id}&channel_name=${this.current.name}&pay_name=${this.current.nick_name}&pay_type=${this.current.pay_type}&client=${this.app.UrlData.client}&proxy_user_id=${this.app.UrlData.proxy_user_id}&proxy_name=${decodeURI(this.app.UrlData.proxy_name)}&package_id=${this.app.UrlData.package_id}&order_ip=${this.login_ip ? this.login_ip:"127.0.0.1"}&device_id=${hqq.app.deviceID}&token=${this.app.token}&center_auth=${this.app.login_token}`;
        cc.find('payGlobal').getComponent('payGlobal_1').imWebViewUrl = encodeURI(url)
        cc.director.preloadScene("payIM",()=>{
            hqq.reflect.setOrientation("portrait", 640, 1136)
    ...
    setLanguageResource(){
        ...
        if(this.app.UrlData.package_id != 15 && this.app.UrlData.package_id != 20 && this.app.UrlData.package_id != 16 && this.app.UrlData.package_id != 18 && this.app.UrlData.package_id != 12 && this.app.UrlData.package_id != 22){
    ...
    blinkFun(blinkNode){
        ...
        this.Interval= setInterval(()=>{
            ...
            blinkNode.getComponent(cc.Label).color = new cc.Color(R,G,B,255)
        },10)
    }
}
