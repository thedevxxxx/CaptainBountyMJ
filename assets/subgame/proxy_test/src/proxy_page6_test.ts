...
import { KsWdblPopP6Test } from './p6/ks_wdbl_pop_p6_test';
import { ZjzrPopTest } from './p6/zjzr_pop_test';
import { KsXjblPopP6Test } from './p6/ks_xjbl_pop_p6_test';
const { ccclass, property } = _decorator;

@ccclass('ProxyPage6Test')
export class ProxyPage6Test extends Component {
    @property(Node)
    ...
    @property(Label)
    public id: cc.Label | null = null;
    @property(Label)
    public date: cc.Label | null = null;
    @property(Label)
    public sx: cc.Label | null = null;
    @property(Label)
    public sfks: cc.Label | null = null;
    @property(Node)
    public MyLossDividend: cc.Node | null = null;
    @property(Node)
    public xjbl_btn: cc.Node | null = null;
    @property(Node)
    public fs_view: cc.Node | null = null;
    @property
    public checkedItemIndex = [];
    ...
    onLoad () {
        ...
        function check() {
           return new Promise((resolve, reject) => {
               commonVal.huaxing_getDividendRule(resolve);
           })
        }
        check().then(
           (value) => {
               cc.log(Database.hx_p6_gz, '==================hx_p6_gz');
           },
           function (error) {
               cc.error("出错了", error);
           },
    ...
    checkMyAgent (event: any, num: any) {
        ...
        cc.log('进入 第六页 第', commonVal.gametags, '游戏', 'token', commonVal.token);
        ...
        commonVal.getchild(hqq.gameGlobal.player.account_name, a);//获取下级id
        commonVal.getDividendRuleonetype(0, 3, a) //为获取冗余的参数
        ...
        function check() {
           return new Promise((resolve, reject) => {
               cc.log('6请求第一页数据');
               commonVal.GetPaymentDividendInfo(num, firstdate, lastdate, resolve)
           })
        }
        check().then(
           (value) => {
               cc.log('youshuju',);
               if (hqq.app.pinpai == 'fuxin' || hqq.app.pinpai == 'juding' || hqq.app.pinpai == 'huangshi') {
                   commonVal.p6sgetDividendRule()
               }
               this.data = Database.getData3();
    ...
    reset () {
        ...
        cc.find("label7", this.MyLossDividend).getComponent(cc.Label).string = '' //期初金额
        cc.find("label9", this.MyLossDividend).getComponent(cc.Label).string = '' //期末金额
        this.sfks.string = ''
    ...
    setMyItemData () {
        ...
        if (data != undefined && data != null) {
           ...
           if (parseFloat(data.money) >= 0) {
               cc.find("label5", this.MyLossDividend).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.money.toFixed(8)))  //应发分红 //应发分红
           } else {
               cc.find("label5", this.MyLossDividend).getComponent(cc.Label).string = '0' //应发分红
           }
           cc.find("label1", this.MyLossDividend).getComponent(cc.Label).string = sjzq; //周期
           cc.find("label8", this.MyLossDividend).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.top_up.toFixed(8))) //总充值
           cc.find("label2", this.MyLossDividend).getComponent(cc.Label).string = Math.abs(parseInt(data.withdraw.toFixed(8)))+''; //总兑换
           cc.find("label7", this.MyLossDividend).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.first_balance.toFixed(8)));//期初金额
           cc.find("label9", this.MyLossDividend).getComponent(cc.Label).string = Database.countCoinsShowLabel(Math.abs(data.last_balance.toFixed(8)));//期末金额
           let liushui = parseFloat(data.top_up) - Math.abs(parseFloat(data.withdraw)) + parseFloat(data.first_balance) - Math.abs(data.last_balance.toFixed(8)) - parseFloat(data.amount)
           cc.log('流水分红扣除', parseFloat(data.top_up), Math.abs(parseFloat(data.withdraw)), parseFloat(data.first_balance), parseFloat(data.last_balance), parseFloat(data.amount), liushui);
           cc.find("label3", this.MyLossDividend).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(liushui.toFixed(8)) );//流水分红扣除
           cc.find("label4", this.MyLossDividend).getComponent(cc.Label).string =  Database.countCoinsShowLabel(data.cost_money.toFixed(8)) //渠道费用扣除
           cc.find("label6", this.MyLossDividend).getComponent(cc.Label).string = datazt //状态
        }
    }

    setItemData (data: any, i: any) {
        ...
        if (parseFloat(data.money) >= 0) {
           cc.find("yffh", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.money.toFixed(8))); //应发分红
        } else {
           cc.find("yffh", newItem).getComponent(cc.Label).string = "0"//应发分红
        }
        cc.find("xjd", newItem).getComponent(cc.Label).string = data.id;//下级id
        cc.find("tdzdh", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(Math.abs(parseFloat(data.withdraw.toFixed(8))));//团队总兑换
        cc.find("tuzcz", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.top_up.toFixed(8)));//团队总充值
        cc.find("zq", newItem).getComponent(cc.Label).string = sjzq; //周期
        cc.find("qcje", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.first_balance.toFixed(8))); //期初金额
        cc.find("qmje", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(Math.abs(data.last_balance.toFixed(8))); //期末金额
        let liushui = parseFloat(data.top_up) - Math.abs(parseFloat(data.withdraw)) + parseFloat(data.first_balance) - Math.abs(data.last_balance.toFixed(8)) - parseFloat(data.amount)
        cc.log('流水分红扣除1', parseFloat(data.top_up), Math.abs(parseFloat(data.withdraw)), parseFloat(data.first_balance), parseFloat(data.last_balance), parseFloat(data.amount), liushui);
        cc.find("lsfhkc", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(liushui.toFixed(8)) );//流水分红扣除
        cc.find("qdfykc", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(data.cost_money.toFixed(8))//渠道费用扣除
    ...
    searchByDate () {
        ...
        if (hqq.app.pinpai == "nineone") {
           ...
           } else if (parseInt(nowyear+'') == parseInt(a[0]) && parseInt(a[1]) - parseInt(nowmonth+'') == -1 && parseInt(nowday+'') <= 7 && this.sx.string == '下') {
               fifteenDays = true;
           } else if (parseInt(nowyear+'') - parseInt(a[0]) == 1 && parseInt(a[1]) == 12 && parseInt(nowmonth+'') == 1 && parseInt(nowday+'') <= 7 && this.sx.string == '下') {
               fifteenDays = true;
           }
        } else {
           if (parseInt(nowyear+'') == parseInt(a[0]) && parseInt(a[1]) == nowmonth) {
        ...
        if (fifteenDays) {
           ...
           function check() {
               return new Promise((resolve, reject) => {
                   commonVal.GetPaymentDividendInfo(commonVal.gametags, dates, enddate, resolve)
               })
           }
           check().then(
               (value) => {
                   cc.log('进来了 check().then', Database.getData3());
                   this.data = [];
                   this.data = Database.getData3();
                   this.pageNodes = [];
                   this.currentPage = 1; //当前页码
                   ...
                   this.setallmoney();
                   let searchdata = [];
                   if (ids != '' && ids != hqq.gameGlobal.player.account_name) {
                       cc.log('ids==', ids);
                       searchdata = Database.searchksByIdp6(ids)  //对应id下级所有数组
                       let alldata = []
                       alldata = [this.data[0]];
    ...
    dispatch (event: any, flag: any) {
        ...
        if (flag == "0") {//一键发放
           ...
           cc.log("Ejfs_______", sendArr)
        } else {//选择发放
           for (var i = 0; i < this.checkedItemIndex.length; ++i) {
               sendArr.push(this.data[this.checkedItemIndex[i]]._id);//假设根据id发放
           }
           cc.log("xzfs______", sendArr)
           if (sendArr.length == 0) {
               cc.log('请选择需要发放的分红信息');
               hqq.eventMgr.dispatch(hqq.eventMgr.showTip, '请选择需要发放的分红信息')
           }
           let ic = 0
    ...
    checkClick (event: any, args: any) {
        ...
        cc.log('被选中列表', this.checkedItemIndex);
    }
    ...
    mybl () {
        ...
        wdbl_pop.getComponent(KsWdblPopP6Test).getDividendRule(hqq.gameGlobal.player.account_name)
    }
    ...
    xjbl () {
        ...
        this.mid.active = true
        cc.log('下级比例');
        xjbl_pop.getComponent(KsXjblPopP6Test).search()
    }
    ...
    zjzr () {
        ...
        this.mid.active = true
        cc.log('资金转入比例');
        zjzr_pop.getComponent(ZjzrPopTest).setData()
    }

    yjzz () {
        Database.clickSound(Database.hall_sound)
        let yjzz_pop = this.node.getChildByName("yjzz_pop");
        this.mid.active = true
        cc.log('佣金转入');
        yjzz_pop.active = true;
    }

    GrantDeficitDividendByDateAndGameTag () {
        let host = hqq.gameGlobal.proxy.proxy_host
        var xhr_test = new XMLHttpRequest();
        xhr_test.open("POST", host + "/proxy/user/GrantPaymentDividendByDateAndGameTag", true);
        ...
        var sendData = `account_name=${hqq.gameGlobal.player.account_name
           }&token=${commonVal.token}&first_date=${this.firstdates}&last_date=${this.lastdates}&game_tag=0&demand_type=${Database.yf_demand_type}&demand_tag=${Database.yf_demand_tag}`;
        cc.log('sendData====', host + "/proxy/user/GGrantPaymentDividendByDateAndGameTag" + sendData);
    ...
    GrantDeficitDividendByRoundID (round_id: any) {
        let host = hqq.gameGlobal.proxy.proxy_host
        var xhr_test = new XMLHttpRequest();
        xhr_test.open("POST", host + "/proxy/user/GrantPaymentDividendByRoundID", true);
        xhr_test.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        var sendData = `account_name=${hqq.gameGlobal.player.account_name
        ...
        xhr_test.onreadystatechange = () => {
           if (xhr_test.readyState == 4 && xhr_test.status === 200) {
               ...
               if (resData.code == 200) {
                   hqq.eventMgr.dispatch(hqq.eventMgr.showTip, '发放成功！')
                   this.searchByDate()
               }
               if (resData.code == 404 && resData.msg == "round id is empty") {
                   ...
                   hqq.eventMgr.dispatch(hqq.eventMgr.showTip, '发放失败！')
               }
           }
           xhr_test.abort();
        };
...
// 
//                 //     if (Database.hx_p6_gz != null && Database.hx_p6_gz > 31) {
//                 //         this.xjbl_btn.active = false
//                 //     }
//                 // }
//             },
...
//         //         } else {
//         //              gHandler.eventMgr.dispatch(gHandler.eventMgr.showTip, "获取数据失败");
//         //         }
//         //     }
//         //     xhr.abort();
//         // };
...
//             (value) => {
//                 cc.log('youshuju',);
//                 if (gHandler.app.pinpai == 'fuxin' || gHandler.app.pinpai == 'juding' || gHandler.app.pinpai == 'huangshi') {
//                     commonVal.p6sgetDividendRule()
//                 }
// 
//                 this.data = Database.getData3();
...
//             cc.find("label9", this.MyLossDividend).getComponent(cc.Label).string = Database.countCoinsShowLabel(Math.abs(data.last_balance.toFixed(8)));//期末金额
// 
//             // 团队总充值 （ top_up）    —      团队总兑换 abs( withdraw)    ＋    期初金额 （ first_balance）  —  期末金额 （ last_balance ） —  总利润（amount）
// 
//             let liushui = parseFloat(data.top_up) - Math.abs(parseFloat(data.withdraw)) + parseFloat(data.first_balance) - Math.abs(data.last_balance.toFixed(8)) - parseFloat(data.amount)
// 
//             cc.log('流水分红扣除', parseFloat(data.top_up), Math.abs(parseFloat(data.withdraw)), parseFloat(data.first_balance), parseFloat(data.last_balance), parseFloat(data.amount), liushui);
// 
// 
//             cc.find("label3", this.MyLossDividend).getComponent(cc.Label).string = Database.countCoinsShowLabel(liushui.toFixed(8));//流水分红扣除
//             //cc.find("label4", this.MyLossDividend).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.cost_money + data.amount * 0 + data.top_up * Database.base_dividend_n).toFixed(8)) //渠道费用扣除
//             cc.find("label4", this.MyLossDividend).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.cost_money).toFixed(8)) //渠道费用扣除
// 
//             cc.find("label6", this.MyLossDividend).getComponent(cc.Label).string = datazt //状态
// 
// 
...
// 
// 
//         //top_up * n 是团队总充值* 对应百分比
//         // cc.find("qdfykc", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.cost_money + data.amount * 0 + data.top_up * Database.base_dividend_n).toFixed(8));//渠道费用扣除
// 
//         cc.find("qdfykc", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(parseFloat(data.cost_money ).toFixed(8));//渠道费用扣除
// 
//         cc.find("zt", newItem).getComponent(cc.Label).string = datazt;
//         // // 
...
//         //     cc.find("dllzks", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(Math.abs(parseFloat(data.first_balance))); //期初金额
//         // } else {
...
//         cc.log(nowyear, nowmonth, nowday);
//         let fifteenDays = false;
//         if (gHandler.app.pinpai == "nineone") {
//             //同年同月
//             if (parseInt(nowyear) == parseInt(a[0]) && parseInt(a[1]) == nowmonth) {
//                 fifteenDays = true;
//                 //上半个月限制 22日之前
...
//         this.mid.active = true
//         //实际用 
//         wdbl_pop.getComponent("ks_wdbl_pop_p6_test").getDividendRule(gHandler.gameGlobal.player.account_name)
// 
// 
// 
//     },
...
//         this.mid.active = true
//         cc.log('下级比例');
//         xjbl_pop.getComponent("ks_xjbl_pop_p6_test").search()
// 
//     },
//     //资金转入
//     zjzr() {
...
//         this.mid.active = true
//         cc.log('资金转入比例');
//         zjzr_pop.getComponent("zjzr_pop_test").setData()
//     },
//     //佣金转入
//     yjzz() {
//         //音效
...
//     GrantDeficitDividendByDateAndGameTag() {
//         // 单个单号亏损分红
//         // URL http://161.117.178.174:12350/proxy/user/GrantPaymentDividendByDateAndGameTag
//         // METHOD POST
//         // PARAMS {
//         //     account_name 用户ID
//         //     token 密匙
