 ...
 export let Database = {
    ...
    sortbydata(map) {
        ...
        Object.keys(map).forEach((key) => {
            // console.log(key, this.StatementDividendInfo[key]);
            keys.push(key);
        });
    ...
    formatData1(msg) {
        ...
        // console.log('formatData1', this.data1);
    },
    ...
    /**
       ...
       * @param coins 用户金币
       */
    countCoinsShowLabel(coins: number) {
        let a = parseFloat((coins * 100).toFixed(10))
        let coinsStr = Math.floor(a) / 100;
        return coinsStr + '';
    },
    /**
    * 金币数值若小数点后为0,则不显示小数点 规则：请以后台所显示的金额小数点后第7位四舍五入到第6位，然后截取小数点后二位
    ...
    */
    ...
    Sb_DATA(a) {
        ...
        if (day- a  >= 1) {
            console.log('不同日');
            first_day = (day - a ).toString();
            first_month = parseInt(month+'') .toString();
            first_year = parseInt(year+'').toString();
        }
        //同年不同月
        if (parseInt(day+'') - a < 1 && parseInt(month+'') - 1 >= 1) {
            let last_days = this.getDaysOfMonth(year, month - 1)
            first_day = (last_days + parseInt(day+'') - a)+'';
            first_month = (parseInt(month+'') - 1).toString();
            first_year = parseInt(year+'').toString();
        ...
        if (parseInt(day+'') - a < 1 && parseInt(month+'') - 1 == 0) {
            ...
            first_day = (last_days + parseInt(day+'') - a).toString();
            first_month = 12+'';
            first_year = (parseInt(year+'') - 1).toString();
        }
        if (parseInt(first_day ) < 10) {
            first_day = '0' + first_day
        }
        if (parseInt(first_month) < 10) {
