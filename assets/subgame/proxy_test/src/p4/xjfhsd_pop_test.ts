...
import { WdksfhLabelTest } from './wdksfh_label_test';
import { GzsdScriptTest } from './gzsd_script_test';
const { ccclass, property } = _decorator;

@ccclass('XjfhsdPopTest')
export class XjfhsdPopTest extends Component {
    @property(Node)
    ...
    setdata(obj: any[]) {
                ...
                btn.on("touchend", (eve) => { //绑定函数
                    ...
                    if (hqq.app.pinpai == 'chaofan') {
                        if (parseFloat(Database.hs_p4_gz) - parseFloat(percents) < 5) {
                            ...
                            hqq.eventMgr.dispatch(hqq.eventMgr.showTip, '超出区间,修改失败')
                            return
                        }
                    }
        ...
        this.new_gz.on("touchend", (event) => {
            ...
            gzsd_pop.getComponent(GzsdScriptTest).savedata(this.now_id, this.gz_length)
            gzsd_pop.getComponent(GzsdScriptTest).setdata() //设置规则设定中自己的界面
        ...
        xhr_test.onreadystatechange = () => {
           if (xhr_test.readyState == 4 && xhr_test.status === 200) {
               ...
               if (resData.code != 200) {
                   ...
                   EdBox_proportion.string =  (Math.floor(resData.msg.percent * 0.05 * 0.5 * 100 * 100) / 100)+'';
...
//             gzsd_pop.getComponent("gzsd_script_test").savedata(this.now_id, this.gz_length)
// 
//             gzsd_pop.getComponent("gzsd_script_test").setdata() //设置规则设定中自己的界面
//             //  this.node.active = false;
//             gzsd_pop.active = true;
// 
//         })
