...
@ccclass('P10ZsmxPopTest')
export class P10ZsmxPopTest extends Component {
    @property(Node)
    ...
    setdata(i: any) {
        console.log('i===',i,'Database.p10_zsmxdata.length==',Database.p10_zsmxdata.length);
        
        if (Database.p10_zsmxdata.length >= i) {
            console.log('data=', Database.p10_zsmxdata[i]);
            let d = JSON.parse(Database.p10_zsmxdata[i])

            let date = d.date
            let ids = d.ids
            let page = d.num

            console.log('date=', date, 'ids=', ids, 'page=', page);
    ...
    setdatas(date: any, ids: any, page: any) {
    ...
    agentDetails(event: any) {
        if (event.getScrollOffset().y > (470 * Database.p10_page) - 500) {
            ...
            this.setdatas(Database.p10_dates, Database.p10_ids, Database.p10_page)
        }
    }
 ...
 */
// let Database = require("../Database_test");
