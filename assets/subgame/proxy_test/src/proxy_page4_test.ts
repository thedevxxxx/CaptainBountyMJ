...
import { XjfhmxPopTest } from './p4/xjfhmx_pop_test';
import { WdblPopTest } from './p4/wdbl_pop_test';
import { XjblPopTest } from './p4/xjbl_pop_test';
const { ccclass, property } = _decorator;

@ccclass('ProxyPage4Test')
export class ProxyPage4Test extends Component {
    @property(Node)
    ...
    checkMyAgent (event: any, num: any) {
     ...
     careful () {
        ...
        for (let i = 0; i < Database.game_tag.length; i++) {
           ...
           if (hqq.app.pinpai == 'nineone' && Database.game_tag[i] == 5) {
               game_5.active = true
           }
           if (hqq.app.pinpai == 'huangshi' && Database.game_tag[i] == 5) {
               game_5.active = true
           }
        }
    ...
    checkMyAgents (event: any, num: any) {
        ...
        function check() {
            return new Promise((resolve, reject) => {
                ...
                let month = date.getMonth() + 1+'';
                let day = date.getDate()+'';
                let newday = parseFloat (day )- 1+''
                month = parseFloat (month ) < 10 ? '0' + month : month
                day = parseFloat (day ) < 10 ? '0' + day : day
                function mGetDate(year, month) { //通过年月 获取准确天数
                    var d = new Date(year, month, 0);
                    return d.getDate();
                }
                let firstdate
                if (parseFloat (newday ) == 0) {
                    newday = mGetDate(year, (parseFloat (month ) - 1))+''
                    let newmonth = (parseFloat (month ) - 1) < 10 ? '0' + (parseFloat (month ) - 1) : (parseFloat (month ) - 1) 
                    firstdate = year + "-" + newmonth + "-" + newday
                    if (parseFloat (month ) == 1) {
                        firstdate = (year - 1) + "-" + 12 + "-" + newday
                    }
                } else {
                    newday = parseFloat (newday ) < 10 ? '0' + newday : newday
    ...
    setItemData (data: any) {
        ...
        cc.find("wdfh", newItem).getComponent(cc.Label).string =  cc.find("wdfh", newItem).getComponent(cc.Label).string = Database.countCoinsShowLabel(data.money - xjfh);
        if (Database.countCoinsShowLabel(data.money - xjfh) == '0') {
            datazt = ''
         }
        ...
        btn.on("touchend", () => {
           ...
           xjfhmx_pop.getComponent(XjfhmxPopTest).setdata(data.date, allData[data.date])
        })
    ...
    mybl () {
        ...
        this.mid.active = true;
        wdbl_pop.getComponent(WdblPopTest).getDividendRule(hqq.gameGlobal.player.account_name)
    }
    ...
    xjbl () {
        ...
        xjbl_pop.getComponent(XjblPopTest).search()
    }
...
//         //用所有当日数据来算数据
//         let alldatas = Database.getAllData1()
// console.log("条目数据==",alldatas,data.date);
//         for (let index = 1; index < alldatas[data.date].length; index++) {
//             xjfh += alldatas[data.date][index].money
//         }
// 
...
//             let allData = Database.getAllData1()
//             this.mid.active = true;
//             xjfhmx_pop.getComponent("xjfhmx_pop_test").setdata(data.date, allData[data.date])
//         })
//         this.sv.content.addChild(newItem);
// 
//     },
...
//         //实际用 
//         this.mid.active = true;
//         wdbl_pop.getComponent("wdbl_pop_test").getDividendRule(gHandler.gameGlobal.player.account_name)
// 
//         //测试用
...
//         let xjbl_pop = this.node.getChildByName("xjbl_pop");
// 
//         xjbl_pop.getComponent("xjbl_pop_test").search()
// 
// 
//     },
