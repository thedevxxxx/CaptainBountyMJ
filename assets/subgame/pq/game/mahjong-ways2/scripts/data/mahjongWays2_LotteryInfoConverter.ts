import { convertServerCodeToSymbolName } from "../converter/mahjongWays2_Converter";
...
import { isGoldenMahjong, isScatter } from "../predictor/mahjongWays2_Predictor";
import { ReelResult, RoundResult, SymbolName, SymbolResult } from "../type/mahjongWays2_Types";
import mahjongWays2_DataRepository from "./mahjongWays2_DataRepository";
import { log } from "cc";

export default class mahjongWays2_LotteryInfoConverter {
    ...
    // public constructor(mahjongWays2_EventRepository: mahjongWays2_EventRepository, mahjongWays2_DataRepository: mahjongWays2_DataRepository) {
    ...
    //     mahjongWays2_EventRepository.onLotteryInfo.Attach((lotteryInfo) => this.onLotteryInfo(lotteryInfo));
    //     mahjongWays2_EventRepository.onReceiveFreeGameRewardClicked.Attach(() => this.previousFinalFreeCount = null);
    ...
    private onLotteryInfo(lotteryInfo: proto.ILotteryInfo) {
        ...
        if (isFirstFreeGame) {
            this.mahjongWays2_DataRepository.setFreeGameTotalWin(0);
        }
        if (isFreeGame) {
            const currentRoundTotalWin = (accumulatedWins.length) > 0 ? (accumulatedWins[accumulatedWins.length - 1]) : 0;
            if (currentRoundTotalWin > 0) {
                const freeGameTotalWin = (this.mahjongWays2_DataRepository.freeGameTotalWin + currentRoundTotalWin);
                this.mahjongWays2_DataRepository.setFreeGameTotalWin(freeGameTotalWin);
            }
        }
        ...
        const freeGameTotalWin = this.mahjongWays2_DataRepository.freeGameTotalWin;
        this.mahjongWays2_EventRepository.onResult.Notify({
    ...
    private logEmoji(symbolsByReelIndex: Array<Array<number>>, isGoldArray: Array<Array<boolean>>) {
        ...
        let emojiStrings = "";
        const symbolCount = symbolsByReelIndex[1].length;//第二排 5个
