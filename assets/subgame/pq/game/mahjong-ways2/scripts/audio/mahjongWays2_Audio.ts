...
export class mahjongWays2_Audio {

    private node: Node;

    private audioPlayers: Array<mahjongWays2_AudioPlayer>;

    private bgmAudioPlayer: mahjongWays2_AudioPlayer;
    ...
    public constructor(node: Node) {
        this.isMute = false;
        this.node = node;
        this.audioPlayers = new Array<mahjongWays2_AudioPlayer>();
    ...
    public playBGM(audioClip: AudioClip) {
        ...
        if (audioPlayer.isPlaying && (audioPlayer.audioSource.clip === audioClip)) {
            console.log("[mahjongWays2_Audio] same bgm");
            return;
        }
        console.log(`[mahjongWays2_Audio] playBGM ${audioClip.name}`);
        audioPlayer.stop();
    ...
    public playByClipPart(clipPart: ClipPart, onCompletedOnce?: Function) {
        if (clipPart == null) {
            console.log("[mahjongWays2_Audio] clipPart is null when executing playByClipPart");
            return;
        }
    ...
    private getAudioPlayer(): mahjongWays2_AudioPlayer {
        let audioPlayer: mahjongWays2_AudioPlayer;
    ...
    private createAudioPlayer() {
        ...
        const audioPlayer = new mahjongWays2_AudioPlayer({
