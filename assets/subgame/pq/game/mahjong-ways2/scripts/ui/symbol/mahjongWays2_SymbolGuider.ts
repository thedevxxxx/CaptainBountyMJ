...
import { SymbolName } from "../../type/mahjongWays2_Types";
...
export default class mahjongWays2_SymbolGuider {
    ...
    public constructor(reelsRootNode: Node, mahjongWays2_UIFactory: mahjongWays2_UIFactory, mahjongWays2_GameState: mahjongWays2_GameState, mahjongWays2_EventRepository: mahjongWays2_EventRepository, mahjongWays2_DataRepository: mahjongWays2_DataRepository) {
        ...
        this.createUI(reelsRootNode, mahjongWays2_UIFactory);
        this.hideBackground();
    ...
    public toggleInfo(target: Node, reelIndex: number, symbolName: SymbolName) {
        if (this.mahjongWays2_DataRepository.freeGameCount > 0) {
            return;
        }
    ...
    private createUI(reelsRootNode: Node, mahjongWays2_UIFactory: mahjongWays2_UIFactory) {
        const background = mahjongWays2_UIFactory.createSprite({
            parent: reelsRootNode,
            name: "mahjongWays2_SymbolGuiderBackground"
        }, {
            sizeMode: Sprite.SizeMode.CUSTOM,
            type: Sprite.Type.SIMPLE,
            spriteFramePath: "mahjong-ways2/images/mahjongWays2_default_sprite_splash/spriteFrame"
        }, {
            top: 0,
            bottom: 0,
            left: 0,
        ...
        const guiderBackground = mahjongWays2_UIFactory.createSprite({
            parent: null,
            name: "mahjongWays2_SymbolGuider"
        }, {
            contentSize: new Size(280, 180),
            sizeMode: Sprite.SizeMode.CUSTOM,
            type: Sprite.Type.SIMPLE,
            spriteFramePath: "mahjong-ways2/images/mahjongWays2_mj_info/spriteFrame"
        }, {

        });
        this.guiderBackgroundNode = guiderBackground.node;

        const descriptionLabel = mahjongWays2_UIFactory.createLabel({
        ...
        const paytablelayout = mahjongWays2_UIFactory.createTransform({
            parent: guiderBackground.node,
            name: "paytablelayout"
        }, {
        ...
        mahjongWays2_UIFactory.addGridLayout(paytablelayout.node, {
        ...
        for (let index = 0; index < 6; index++) {
            mahjongWays2_UIFactory.createLabel({
                parent: paytablelayout.node,
                name: "paytableLable",
            }, {
                anchorPoint: new Vec2(0, 0.5),
                fontPath: (index > 2) ? "mahjong-ways2/bigImage/text/num/a_mahjongWays2_silver_num" : "mahjong-ways2/bigImage/text/num/a_mahjongWays2_gold_num",
                color: Color.WHITE,
                spacingX: -12,
                fontSize: 30,
                string: "0"
    ...
    private showInfo(target: Node, reelIndex: number, symbolName: SymbolName) {
        if (symbolName == null) {
            console.log("[mahjongWays2_SymbolGuider] symbolName is null");
            return;
        }
        this.hideInfo();
        this.showBackground();
        ...
        this.mahjongWays2_EventRepository.onSymbolGuiderShow.Notify();
    }
