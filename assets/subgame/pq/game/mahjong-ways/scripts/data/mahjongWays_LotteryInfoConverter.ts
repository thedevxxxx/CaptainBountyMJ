...
import { isGoldenMahjong, isScatter } from "../predictor/mahjongWays_Predictor";
import { ReelResult, RoundResult, SymbolName, SymbolResult } from "../type/mahjongWays_Types";
import mahjongWays_DataRepository from "./mahjongWays_DataRepository";
import { log } from "cc";

export default class mahjongWays_LotteryInfoConverter {
    ...
    // public constructor(mahjongWays_EventRepository: mahjongWays_EventRepository, mahjongWays_DataRepository: mahjongWays_DataRepository) {
    //     this.mahjongWays_DataRepository = mahjongWays_DataRepository;
    //     this.mahjongWays_EventRepository = mahjongWays_EventRepository;
    //     mahjongWays_EventRepository.onLotteryInfo.Attach((lotteryInfo) => this.onLotteryInfo(lotteryInfo));
    //     mahjongWays_EventRepository.onReceiveFreeGameRewardClicked.Attach(() => this.previousFinalFreeCount = null);
    ...
    private onLotteryInfo(lotteryInfo: proto.ILotteryInfo) {
        ...
        if (isFirstFreeGame) {
            this.mahjongWays_DataRepository.setFreeGameTotalWin(0);
        }
        if (isFreeGame) {
            const currentRoundTotalWin = (accumulatedWins.length) > 0 ? (accumulatedWins[accumulatedWins.length - 1]) : 0;
            if (currentRoundTotalWin > 0) {
                const freeGameTotalWin = (this.mahjongWays_DataRepository.freeGameTotalWin + currentRoundTotalWin);
                this.mahjongWays_DataRepository.setFreeGameTotalWin(freeGameTotalWin);
            }
        }
        ...
        const freeGameTotalWin = this.mahjongWays_DataRepository.freeGameTotalWin;
        this.mahjongWays_EventRepository.onResult.Notify({
