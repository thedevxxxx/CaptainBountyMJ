...
export class mjhl_Audio {

    private node: Node;

    private audioPlayers: Array<mjhl_AudioPlayer>;

    private bgmAudioPlayer: mjhl_AudioPlayer;
    ...
    public constructor(node: Node) {
        this.isMute = false;
        this.node = node;
        this.audioPlayers = new Array<mjhl_AudioPlayer>();
    ...
    public playBGM(audioClip: AudioClip) {
        ...
        if (audioPlayer.isPlaying && (audioPlayer.audioSource.clip === audioClip)) {
            console.log("[mjhl_Audio] same bgm");
            return;
        }
        console.log(`[mjhl_Audio] playBGM ${audioClip.name}`);
        audioPlayer.stop();
    ...
    public playByClipPart(clipPart: ClipPart, onCompletedOnce?: Function) {
        if (clipPart == null) {
            console.log("[mjhl_Audio] clipPart is null when executing playByClipPart");
            return;
        }
    ...
    private getAudioPlayer(): mjhl_AudioPlayer {
        let audioPlayer: mjhl_AudioPlayer;
    ...
    private createAudioPlayer() {
        ...
        const audioPlayer = new mjhl_AudioPlayer({
