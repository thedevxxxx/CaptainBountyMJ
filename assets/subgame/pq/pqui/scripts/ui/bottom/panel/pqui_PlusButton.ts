...
export default class pqui_PlusButton {
    ...
    public constructor(parent: Node, pqui_UIFactory: pqui_UIFactory, pqui_DataRepository: pqui_DataRepository, pqui_EventRepository: pqui_EventRepository) {
        ...
        const button = pqui_UIFactory.createButton({
            ...
            spriteFramePath: "pq/pqui/images/pqui_btn_plus/spriteFrame",
        }, null, {
            right: 170,
            top: 22.5
        });
        ...
        pqui_EventRepository.onTotalBetAmountChangedUI.Attach((totalBet) => {
            const totalBets = this.pqui_DataRepository.totalBets;
            if (totalBet === totalBets[totalBets.length - 1]) {
    ...
    private onPlusButtonClicked() {
        ...
        if (currentTotalBet === totalBets[totalBets.length - 1]) {
            index = totalBets.length - 1;
            this.pqui_EventRepository.onBlackToastUI.Notify({ content: "最大投注" });
        } else {
