...
hqqWebSocket2.prototype = {
    ...
    connect(param) {
        ...
        this.ip = param || url
        if (cc.sys.platform === cc.sys.Platform.ANDROID || cc.sys.os === cc.sys.OS.ANDROID) { //在浏览器调试中，cc.sys.os === cc.sys.OS.ANDROID 是true  || cc.sys.os == cc.sys.OS_WINDOWS
            if (this.ip.indexOf('wss') == -1 || cc.sys.isBrowser) {
                this.ws = new WebSocket(this.ip);
            } else {
                cc.resources.load('hall/cacert',(err:Error,asset:cc.Asset)=>{
                    if(err){
                        cc.log("haallwebsocket 证书无效");
                        return;
                    }
                    //@ts-ignore
                    this.ws = new WebSocket(this.ip, {}, asset.nativeUrl);
    ...
    onReceiveNologin(data) { // 账号掉线
        ...
        hqq.eventMgr.dispatch(hqq.eventMgr.showSamlllayer, { type: 6 , hideexitbtn:true })
    },
