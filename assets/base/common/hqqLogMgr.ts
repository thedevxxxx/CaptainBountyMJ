    ...
    send: function (logstr, islog) {
        ...
        if (hqq.gameGlobal.token != "" && hqq.app.server) {
            ...
            if (cc.sys.isBrowser) {
                ...
                let temppackagename = hqq.app.packgeName.split(".");
                data.package_name = temppackagename[0]+"."+temppackagename[1]+"."+temppackagename[2]+"."+systemstr;
            }
            ...
            if (islog) {
                ...
                this.output += this.getNowTime() + ":send失败," + err + this.tag;
            } else {
                this.eoutput = logstr + this.tag;
                this.eoutput += this.getNowTime() + ":send失败," + err + this.tag;
            }
        }
    },
    setDelaySend() {
