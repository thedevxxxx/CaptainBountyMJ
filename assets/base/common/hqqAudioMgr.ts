...
export let audioMgr = {
    ...
    playAudio(name, type, subbundle ,isMultiTrack=true) {
        if (cc.isValid(this.resMap[name])) {
            if (type == 'bg') {
                if (this.bgIsOpen) {
                    if(cc.isValid(this.bgchip)){
                        if(this.bgchip.name == name ){
                            return;
                        }
                    }
                    this.bgchip = this.resMap[name];
                    if( !cc.isValid(this._Music) ){
                        let node:cc.Node = new cc.Node("music");
                        cc.director.getScene().addChildEx(node);
                        cc.game.addPersistRootNode(node);
                        this._Music = node.addComponent( cc.AudioSource );
                    } else{
                        this._Music.stop();
                    }
    ...
    playBg(name = "hallbg", subbundle = null , musicIndex = null ) {
        hqq.eventMgr.dispatch(hqq.eventMgr.playbg, name, subbundle , musicIndex)
    },
    playBg2(name = "hallbg", subbundle = null , musicIndex = null ) {
        if (name == "hallbg") {
