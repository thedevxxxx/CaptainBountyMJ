...
export let gHandler = {
    ...
    isShowJumpScene:false,//是否是跳轉場景中
    gameGlobal: {
    ...
    spriteResMap: new Map(), //记录载过的图  减少重复加载
    setFuxinHallIdType() {
    ...
    // UI图片动态加载
    imgLoad(node:cc.Node, path:any,Res:cc.AssetManager.Bundle = null) {
        if (!cc.isValid(node)) {
    ...
    // 多语言按钮图片加载
    btnLoad(node:cc.Node, normal, pressed, interactable,Res:cc.AssetManager.Bundle = null ) { //, hover, disabled 只做两个图片
        if(!cc.isValid(node)){
        ...
        tempRes.load(normal, cc.SpriteFrame, (err, frame) => {
            ...
            pressed = pressed ? pressed + "/spriteFrame": normal
            widget && this.setWidget(node, widget)
    ...
    // editbox 提示语设置
    editboxTipLoad(node:cc.Node, tipname) {
        if(!cc.isValid(node)){
    ...
    // 骨骼动画设置
    skeletonLoad(node:cc.Node, path, aniname, loop,Res:cc.AssetManager.Bundle=null) {
        if (node instanceof cc.Node) {
        } else {
        ...
        let ani = node.getComponent(cc.sp.Skeleton)
        if (!ani) {
            // console.warn("节点不含骨骼组件")
            ani = node.addComponent(cc.sp.Skeleton)
        ...
        tempRes.loadDir(path, cc.sp.SkeletonData, (err, Data) => {
            if (err) {
                cc.log("加载骨骼动画失败", err)
                return;
            }
    ...
    // 为节点设置停靠
    setWidget(node:cc.Node, config) { //  top, left, bottom, right, horizontalCenter, verticalCenter
        if (!cc.isValid(node)) {
    ...
    addNode(node:cc.Node, cfg) { // name,
        ...
        if (typeof cfg.zIndex == "number") {
            node.addChildEx(newnode, cfg.zIndex)
        } else {
            node.addChildEx(newnode)
        }
    ...
    setSprite(node:cc.Node, cfg) { // path
        if (!this.checkNode(node)) {
            console.trace("setSprite 节点错误")
            // for (let k in cfg) {
            //     cc.log(k, cfg[k])
            // }
            return
        ...
        if(this.spriteResMap.has(cfg.path)) {
            if (!cc.isValid(node)) {
                return
            }
            sprite.spriteFrame = this.spriteResMap.get(cfg.path);
            ...
            tempRes.load(cfg.path, cc.SpriteFrame, (err, frame) => {
                ...
                this.spriteResMap.set(cfg.path , frame);
                sprite.spriteFrame = frame;
    ...
    setToggle(node:cc.Node, cfg) { // normal == path, pressed, interactable, callback，widget
        if (!cc.isValid(node)) {
            cc.log("setToggle 传入的节点是空", cfg)
            return
        }
        ...
        if (node instanceof cc.Node) {
            ...
            cc.log("setToggle 传入的节点不是cc.node")
            return
        }
        let toggle = node.getComponent(cc.Toggle)
        if (!toggle) {
            toggle = node.addComponent(cc.Toggle)
        }
        if (cfg.callback) {
            ...
            if(toggle.checkEvents.length > 0 ){
                toggle.checkEvents[0] = clickEventHandler;
            } else{
                toggle.checkEvents.push(clickEventHandler);
            }
        }
    },
    setBtn(node:cc.Node, cfg) { // normal == path, pressed, interactable, callback，widget
        if (!cc.isValid(node)) {
        ...
        if (cfg.callback) {
            ...
            if(btn.clickEvents.length > 0 ){
                btn.clickEvents[0] = clickEventHandler;
            } else{
        ...
        if (btn.transition == cc.Button.Transition.SCALE||
            ...
            tempRes.load(cfg.normal, cc.SpriteFrame, (err, frame) => {
                ...
                btn.normalSprite = frame;
                btn.hoverSprite = frame;
                cfg.pressed = cfg.pressed ? cfg.pressed + "/spriteFrame": cfg.normal
    ...
    setSkeleton(node:cc.Node, cfg) { // path aniname loop
        if (!cc.isValid(node)) {
            return
        }
        if (!cfg.path) {
        ...
        // ani.setAnimationCacheMode(cc.sp.Skeleton.AnimationCacheMode.PRIVATE_CACHE);
        ...
        tempRes.loadDir(cfg.path, cc.sp.SkeletonData, (err, Data:cc.sp.SkeletonData[]) => {
            if (err) {
                cc.log("加载骨骼动画失败", err)
                return;
            }
            ...
            for (let i = 0; i < Data.length; i++) {
                cc.log("setSkeleton i=",i, " classname=",cc.js.getClassName(Data[i]))
                if (cc.js.getClassName(Data[i]) == "sp.SkeletonData") {
                    ani.skeletonData = Data[i];
                }
            }
    ...
    setScrollView(node:cc.Node, cfg) {
        if (!this.checkNode(node)) {
        ...
        this.setNode(node, cfg)
        component.view.width = node.getComponent(cc.UITransform).width
        component.view.height = node.getComponent(cc.UITransform).height
        if (cfg.content) {
            component.content.getComponent(cc.UITransform).width = cfg.content.width
            component.content.getComponent(cc.UITransform).height = cfg.content.height
        } else {
            component.content.getComponent(cc.UITransform).width = node.getComponent(cc.UITransform).width
            component.content.getComponent(cc.UITransform).height = node.getComponent(cc.UITransform).height
        }
    },
    // 检查节点
    checkNode(node:cc.Node) {
        if (!cc.isValid(node)) {
            console.trace("传入的节点是空")
            return false
        }
        if (node instanceof cc.Node) {
        } else {
    ...
    // 设置节点属性
    setNode(node:cc.Node, cfg) {
        if (!this.checkNode(node)) {
            cc.log("setNode 节点错误")
            return
        }

        if(!node.getComponent(cc.UITransform)){
        ...
        if (cfg.size) {
            if (cfg.size.width) {
                node.getComponent(cc.UITransform).width = cfg.size.width
            }
            if (cfg.size.height) {
                node.getComponent(cc.UITransform).height = cfg.size.height
            }
        }
        if (typeof cfg.width == "number") {
            node.getComponent(cc.UITransform).width = cfg.width
        }
        if (typeof cfg.height == "number") {
            node.getComponent(cc.UITransform).height = cfg.height
        }
        if (cfg.scale) {
            node.scale = cfg.scale
            node.setScaleEx(cfg.scale)
        }
        if (cfg.scaleX) {
            node.setScaleEx(cfg.scaleX,node.getScale().y,node.getScale().z);
        }
        if (cfg.scaleY) {
            node.setScaleEx(node.getScale().x,cfg.scaleY,node.getScale().z);
        }
        if (cfg.anchorX || cfg.anchorX === 0 ) {
            node.getComponent(cc.UITransform).anchorX = cfg.anchorX
        }
        if (cfg.anchorY) {
            node.getComponent(cc.UITransform).anchorY = cfg.anchorY
        }
        ...
        let tempcolor:cc.math.Color = cfg.color || cc.color(255,255,255,255);
        if (cfg.opacity || 0 == cfg.opacity){
            if(node.getComponent(cc.Label)){
                tempcolor = node.getComponent(cc.Label).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.Label).color = tempcolor;
            } else if(node.getComponent(cc.Sprite)){
                tempcolor = node.getComponent(cc.Sprite).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.Sprite).color = tempcolor;
            } else if(node.getComponent(cc.sp.Skeleton)){
                tempcolor = node.getComponent(cc.sp.Skeleton).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.sp.Skeleton).color = tempcolor;
            } else if(node.getComponent(cc.dragonBones.ArmatureDisplay)){
                tempcolor = node.getComponent(cc.dragonBones.ArmatureDisplay).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.dragonBones.ArmatureDisplay).color = tempcolor;
            } else if(node.getComponent(cc.Renderable2D)){
                tempcolor = node.getComponent(cc.Renderable2D).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.Renderable2D).color = tempcolor;
            } else if(node.getComponent(cc.Mask)){
                tempcolor = node.getComponent(cc.Mask).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.Mask).color = tempcolor;
            } else if(node.getComponent(cc.LabelOutline)){
                tempcolor = node.getComponent(cc.LabelOutline).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.LabelOutline).color = tempcolor;
            } else if(node.getComponent(cc.Graphics)){
                tempcolor = node.getComponent(cc.Graphics).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.Graphics).color = tempcolor;
            } else if(node.getComponent(cc.UIStaticBatch)){
                tempcolor = node.getComponent(cc.UIStaticBatch).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.UIStaticBatch).color = tempcolor;
            } else if(node.getComponent(cc.LabelShadow)){
                tempcolor = node.getComponent(cc.LabelShadow).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.LabelShadow).color = tempcolor;
            } else if(node.getComponent(cc.Light)){
                tempcolor = node.getComponent(cc.Light).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.Light).color = tempcolor;
            } else if(node.getComponent(cc.ParticleSystem2D)){
                tempcolor = node.getComponent(cc.ParticleSystem2D).color;
                tempcolor.a = cfg.opacity;
                node.getComponent(cc.ParticleSystem2D).color = tempcolor;
            }
        }
        if(cfg.color){
            if(node.getComponent(cc.Label)){
                ...
                if( !node.getComponent(cc.UIOpacity) ){
                    node.addComponent(cc.UIOpacity);
                }
                if (cfg.opacity || 0 == cfg.opacity){
                    node.getComponent(cc.UIOpacity).opacity = cfg.opacity
                }
            }
        }
    ...
    setVideoPlayer(node, cfg) { // path
        ...
        if (cfg.callback) {
            let videoEventHandler = new cc.EventHandler();
