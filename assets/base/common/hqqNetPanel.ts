...
export class hqqNetPanel extends cc.Component {
    ...
    UILoad() {
        ...
        if (hqq.app.pinpai == "fuxin") {
            ...
            hqq.addNode(btn_quickPick, { string: "ksxz", x: 0, fontSize: 25 })
            if (this.data.choicetype) {
            ...
            hqq.addNode(btn_quickPick, { string: "ksxz", fontSize: 24, color: cc.color(148, 81, 10, 255), bold: true })
            ...
            hqq.setNode(upinto, { x: -130 })
            hqq.setNode(centerinfo, { x: -130 })
            let curupline = cc.find("curline/curlinelabel", up)
            hqq.setNode(curupline, { x: 10 })
            let upspeed = cc.find("curline/speed", up)
            hqq.setNode(upspeed, { x: 110 })
            ...
            hqq.addNode(btn_quickPick, { string: "ksxz", fontSize: 24, bold: true })
            hqq.addNode(btn_refresh, { string: "str12", fontSize: 24, color: cc.color(100, 36, 16,255), bold: true })
            ...
            hqq.addNode(btn_quickPick, { string: "ksxz", fontSize: 24, bold: true, color: cc.color(59, 31, 11,255) })
            ...
            hqq.addNode(background, { path: xpath + "d_tit", y: 300 })
            hqq.setSprite(title_lt, { path: xlpath + "xiankujiance", x: 0, y: 300, })
            hqq.setBtn(exitbtn, { normal: xpath + "btn_x", x: 515, y: 300, callback: "onClickExit", script: this })
            hqq.setBtn(btn_quickPick, { normal: xpath + "btn_2", x: 360, y: 270, callback: "onClickQuikLogin", script: this, width: 159, height: 52 })
            hqq.addNode(btn_quickPick, { string: "ksxz", fontSize: 24, bold: true , y:5})
            hqq.addNode(btn_refresh, { string: "str12", fontSize: 24, color: cc.color(100, 36, 16,255), bold: true })
            hqq.setSprite(center, { path: xpath + "xiaokuang", x: 250, y: -40, width: 486, height: 425, type: cc.Sprite.Type.SLICED })
            hqq.setSprite(txt_t2, { path: xlpath + "yxfwxxl", y: 240 })
            hqq.setNode(upinto, { x: -130 })
            ...
            let tpath = "base/quyou/"
            let tlpath = "base/language/" + hqq.language + "/quyou/img/"
            cc.log("=============== hqqNetPanel background=",background);
            hqq.setSprite(background, { path: tpath+"nfhtr5454" })
            hqq.setSprite(title_lt, { path: tlpath + "gfdgdf" ,y:220})
            hqq.setBtn(exitbtn, { normal: tpath + "bcdrrere", callback: "onClickExit", script: this ,width:120,height:120})
            hqq.setBtn(btn_quickPick, { normal: tpath + "vcbdfer", callback: "onClickQuikLogin", script: this,x:-313,y:220 })
            hqq.addNode(btn_quickPick,{string:"ksxz",fontSize:25,bold:true,color:cc.color(66,45,0),y:5})
            
            this.curUpLine.y = 100;
            this.curCenterLine.y = 100;

            if (this.data.choicetype) {
                hqq.setBtn(btn_refresh, { normal: tpath + "vcbdfer", position: { x: 240, y: 140 }, callback: "onClickRefreshTest", script: this })
                hqq.setSprite(up, { path: tpath + "vcbcrrerekl",width:632,height:397});

                this.upScrollview.node.getComponent(cc.UITransform).setContentSize(632, 250);
                this.upScrollview.node.getComponent(cc.UITransform).width = 632;
                this.upScrollview.node.getComponent(cc.UITransform).height = 250;
                this.upScrollview.view.setContentSize(632, 250);
                this.upScrollview.content.getComponent(cc.UITransform).width = 632;
            } else {
                hqq.setBtn(btn_refresh, { normal: tpath + "vcbdfer", position: { x: 0, y: 185 }, callback: "onClickRefreshTest", script: this })
                hqq.setSprite(up, { path: tpath + "bvcbcvdfr" ,width:984,height:454})
            }
            up.getChildByName("curline").getComponent(cc.Sprite).spriteFrame = null;
            center.getChildByName("curline").getComponent(cc.Sprite).spriteFrame = null;
            this.item.getComponent(cc.Sprite).spriteFrame = null;
            hqq.addNode(btn_refresh,{string:"str12",fontSize:25,bold:true,color:cc.color(66,45,0),y:5});
            txt_t1._removeComponent(txt_t1.getComponent(cc.Sprite));
            hqq.setLabel(txt_t1, { string:"showtip102" })

            txt_t2._removeComponent(txt_t2.getComponent(cc.Sprite));
            hqq.setLabel(txt_t2, { string:"showtip103" } )
            
            hqq.setSprite(center, { path: tpath + "vcbcrrerekl" })
        } else {
            let tpath = "base/img/"
            let tlpath = "base/language/" + hqq.language + "/img/"
            cc.log("=============== hqqNetPanel background=",background);
        ...
        for (let i = 0; i < list.length; i++) {
            if (this.upgradeItemList[i]) {
                ...
                if (hqq.app.pinpai == "fuxin") {
                    ...
                    hqq.addNode(choicebtn, { string: "xz", fontSize: 22 })
                } else if (hqq.app.pinpai == "xinsheng") {
                    ...
                    hqq.addNode(choicebtn, { string: "xz", fontSize: 22, color: cc.color(59, 31, 11,255), bold: true })
                } else if (hqq.app.pinpai == "quyou") {
                    hqq.setBtn(choicebtn, { path: "base/quyou/bveytrg" })
                    hqq.addNode(choicebtn, { string: "xz", fontSize: 22, color: cc.color(66,45,0), bold: true,y:5 })
                } else {
    ...
    getColor(time: any) {
        if (time <= hqq.app.netState.outstanding) {
            return cc.color(146, 255, 24,255)
        } else if (time <= hqq.app.netState.good) {
            return cc.color(255, 185, 29,255)
        } else if (time <= hqq.app.netState.kind) {
            return cc.color(255, 21, 36,255)
        } else if (time <= hqq.app.netState.bad) {
            return cc.color(145, 145, 145,255)
        } else {
