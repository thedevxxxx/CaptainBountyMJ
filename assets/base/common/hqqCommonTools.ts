import * as cc from 'cc';
import { JSB } from 'cc/env';
export let commonTools = {
    // 版本对比
    versionCompare(loaclV, remoteV) {
        if (!loaclV || !remoteV) {
            return true;
        } else {
            let vA = remoteV.split('.');
            let vB = loaclV.split('.');
            for (let i = 0; i < vA.length; ++i) {
                let a = parseInt(vA[i]);
                let b = parseInt(vB[i] || 0);
                if (a != b) {
                    return true;
                }
            }
            if (vB.length != vA.length) {
                return true;
            }
        }
        return false
    },
    headRes: null, // 头像资源缓存
    loadHeadRes(headid,headsprite,size){
        hqq.eventMgr.dispatch(hqq.eventMgr.loadHeadRes, headid,headsprite,size)
    },
    loadHeadRes2(headid:string|number, headsprite:cc.Sprite, size:number) {
        if (typeof headid == "string" && headid.includes(".")) {
            headid = headid.substring(0, headid.indexOf("."))
        }
        
        headid = parseInt(headid.toString()) % 10
        if (hqq.app.pinpai != "xingui"&&hqq.app.pinpai != "ninetwo" && hqq.app.pinpai != "ninethree" && cc.isValid(  this.headRes )) {
            var spriteFrame = this.headRes.getSpriteFrame(`Avatar` + headid)
            if (spriteFrame) {
                headsprite.spriteFrame = spriteFrame;
            } else {
                headsprite.spriteFrame = this.headRes.getSpriteFrame(`Avatar0`);
            }
        } else {
            if (hqq.app.pinpai == "xingui") {
                hqq["hall_xingui"].load(`/xingui/head/tx` + (headid + 1)+"/spriteFrame", cc.SpriteFrame, (err:Error, t:cc.SpriteFrame) => {
                    if(err){
                        cc.log(`/xingui/head/tx` + (Number(headid) + 1)+"/spriteFrame err=",err);
                        return;
                    }
                    if (t) {
                        if( cc.isValid( headsprite ) )
                        {
                            headsprite.spriteFrame = t;
                            if (size) {
                                headsprite.node.getComponent(cc.UITransform).width = size
                                headsprite.node.getComponent(cc.UITransform).height = size
                            }
                        }
                    }
                })
            } else if (hqq.app.pinpai == "ninetwo" || hqq.app.pinpai == "ninethree"){
                let headstr = "x"
                if( size == 101 ){
                    headstr = "d"
                }
                let headconverid = [ 0,0,0,1,1,1,2,2,3,3]
                hqq["hall_"+hqq.app.pinpai].load(hqq.app.pinpai+`/head/`+headstr+headconverid[headid]+"/spriteFrame", cc.SpriteFrame, (err:Error, t:cc.SpriteFrame) => {
                    if(err){
                        cc.log(hqq.app.pinpai+`/head/`+headstr+headconverid[headid]+"/spriteFrame err=",err);
                        return;
                    }
                    if( cc.isValid( headsprite ) )
                    {
                        cc.log("loadHeadRes headid=",headid," headconverid[i]=",headconverid[headid])
                        headsprite.spriteFrame = t;
                        if( size == 101 ){
                            headsprite.node.getComponent(cc.UITransform).width = 101
                            headsprite.node.getComponent(cc.UITransform).height = 155
                        } else if (size) {
                            headsprite.node.getComponent(cc.UITransform).width = size
                            headsprite.node.getComponent(cc.UITransform).height = size
                        }
                    }
                })

            } else if (hqq.app.pinpai == "juding") {
                hqq["hall_juding"].load(`juding/imhead/im_head`, cc.SpriteAtlas, (err:Error, t:cc.SpriteAtlas) => {
                    if(err){
                        cc.log(`juding/imhead/im_head err=`,err);
                        return;
                    }
                    if (t) {
                        if( cc.isValid( headsprite ) )
                        {
                            this.headRes = t
                            var spriteFrame = t.getSpriteFrame(`Avatar` + headid)
                            if (spriteFrame) {
                                headsprite.spriteFrame = spriteFrame;
                            } else {
                                headsprite.spriteFrame = t.getSpriteFrame(`Avatar0`);
                            }
                            if (size) {
                                headsprite.node.getComponent(cc.UITransform).width = size
                                headsprite.node.getComponent(cc.UITransform).height = size
                            }
                        }
                    }
                })
            } else if (hqq.app.pinpai == "xinlong") {
                hqq["hall_xinlong"].load(`xinlong/imhead/im_head`, cc.SpriteAtlas, (err:Error, t:cc.SpriteAtlas) => {
                    if(err){
                        cc.log(`xinlong/imhead/im_head err=`,err);
                        return;
                    }
                    if (t) {
                        if( cc.isValid( headsprite ) )
                        {
                            this.headRes = t
                            var spriteFrame = t.getSpriteFrame(`Avatar` + headid)
                            if (spriteFrame) {
                                headsprite.spriteFrame = spriteFrame;
                            } else {
                                headsprite.spriteFrame = t.getSpriteFrame(`Avatar0`);
                            }
                            if (size) {
                                headsprite.node.getComponent(cc.UITransform).width = size
                                headsprite.node.getComponent(cc.UITransform).height = size
                            }
                        }
                    }
                })
            } else if(hqq.app.pinpai == "wansheng" || hqq.app.pinpai == "letian" ) {
                hqq["hall_" + hqq.app.pinpai].load(hqq.app.pinpai+ `/imhead/im_head`, cc.SpriteAtlas, (err:Error, t:cc.SpriteAtlas) => {
                    if(err){
                        cc.log(hqq.app.pinpai+ `/imhead/im_head err=`,err);
                        return;
                    }
                    if (t) {
                        if( cc.isValid( headsprite ) )
                        {
                            this.headRes = t
                            var spriteFrame = t.getSpriteFrame(`Avatar` + headid)
                            if (spriteFrame) {
                                headsprite.spriteFrame = spriteFrame;
                            } else {
                                headsprite.spriteFrame = t.getSpriteFrame(`Avatar0`);
                            }
                            if (size) {
                                headsprite.node.getComponent(cc.UITransform).width = size
                                headsprite.node.getComponent(cc.UITransform).height = size
                            }
                        }
                    }
                })
            } else if (hqq.app.pinpai == "debi") {
                let headstr = "x"
                if (size == 101) {
                    headstr = "d"
                }
                let headconverid = [0, 0, 0, 1, 1, 1, 2, 2, 3, 3]
                cc.log("loadHeadRes headid=", headid, " head[i]=", headid)
                hqq["hall_debi"].load(`debi/imhead/Avatar` + headid+"/spriteFrame", cc.SpriteFrame, (err:Error, t:cc.SpriteFrame) => {
                    if(err){
                        cc.log(`debi/imhead/Avatar` + headid+"/spriteFrame err=",err);
                        return;
                    }
                    if (cc.isValid(headsprite)) {
                        headsprite.spriteFrame = t;
                        if (size) {
                            headsprite.node.getComponent(cc.UITransform).width = size
                            headsprite.node.getComponent(cc.UITransform).height = size
                        }
                    }
                })
            } else {
                cc.resources.load(`/head/im_head`, cc.SpriteAtlas, (err:Error, t:cc.SpriteAtlas) => {
                    if(err){
                        cc.log("/head/im_head err=",err);
                        return;
                    }
                    if( cc.isValid( headsprite ) )
                    {
                        this.headRes = t
                        var spriteFrame = t.getSpriteFrame(`Avatar` + headid)
                        if (spriteFrame) {
                            headsprite.spriteFrame = spriteFrame;
                        } else {
                            headsprite.spriteFrame = t.getSpriteFrame(`Avatar0`);
                        }
                        if (size) {
                            headsprite.node.getComponent(cc.UITransform).width = size
                            headsprite.node.getComponent(cc.UITransform).height = size
                        }
                    }
                })

            }
        }
    },
    /** 判断是否为数字 */
    isNumber(obj) {
        if (typeof obj === 'number' && !isNaN(obj)) {
            return true;
        }
        return false;
    },
    /** 截屏  注意：web端下载图片到本地待解决
     * @param {object} target 截图组件本身
     * @param {string} fileName 图片名字.jpg（可选）
     * @example
     * hqq.commonTools.screenShot(this); 
     * or hqq.commonTools.screenShot(this，"文件名.jpg");
     */
    screenShot(target, fileName) {
        // // let date = new Date();
        // // let time = "" + date.getMonth() + date.getDate() + date.getHours() + date.getMinutes() + date.getSeconds();
        // let gl = cc.game._renderContext; // gl.STENCIL_INDEX8 质量较高  gl.DEPTH_STENCIL 质量较低
        // let render = new cc.RenderTexture();
        // render.initWithSize(Math.floor(cc.visibleRect.width), Math.floor(cc.visibleRect.height), gl.DEPTH_STENCIL);
        // cc.Camera.main.targetTexture = render;
        // let scaleAction = cc.scaleTo(0.5, 0.3);
        // let targetPos = cc.v2(cc.visibleRect.width - cc.visibleRect.width / 6, cc.visibleRect.height / 4);
        // let moveAction = cc.moveTo(0.5, targetPos);
        // let spawn = cc.spawn(scaleAction, moveAction);
        // let node = new cc.Node();
        // node.setPositionEx(cc.v2(cc.visibleRect.width / 2, cc.visibleRect.height / 2));
        // node.zIndex = cc.macro.MAX_ZINDEX;
        // node.on(cc.Node.EventType.TOUCH_START, () => {
        //     node.parent = null;
        //     node.destroy();
        // });
        // var fileName = fileName || "cocosScreenShot.jpg";
        // if (JSB) {
        //     var fullPath = jsb.fileUtils.getWritablePath() + fileName;
        //     if (jsb.fileUtils.isFileExist(fullPath)) {
        //         jsb.fileUtils.removeFile(fullPath);
        //     }
        //     target.scheduleOnce(() => {
        //         if(!cc.isValid(node))return;
        //         cc.Camera.main.targetTexture = null;
        //         let data = render.readPixels();
        //         let width = render.width;
        //         let height = render.height;
        //         let picData = new Uint8Array(width * height * 4);
        //         let rowBytes = width * 4;
        //         for (let row = 0; row < height; row++) {
        //             let srow = height - 1 - row;
        //             let start = srow * width * 4;
        //             let reStart = row * width * 4;
        //             for (let i = 0; i < rowBytes; i++) {
        //                 picData[reStart + i] = data[start + i];
        //             }
        //         }
        //         jsb.saveImageData(picData, width, height, fullPath);
        //         let texture = new cc.Texture2D();
        //         cc.log(width, height)
        //         texture.initWithData(picData, texture._format, width, height);
        //         let spriteFrame = new cc.SpriteFrame();
        //         spriteFrame.setTexture(texture);
        //         let sprite = node.addComponent(cc.Sprite);
        //         sprite.spriteFrame = spriteFrame;
        //         node.parent = cc.director.getScene();
        //         node.runAction(spawn);
        //     }, 0, 0)
        // } else {
        //     target.scheduleOnce(() => {
        //         if(!cc.isValid(node))return;
        //         cc.Camera.main.targetTexture = null;
        //         if (!this._canvas) {
        //             this._canvas = document.createElement('canvas');
        //             this._canvas.width = render.width;
        //             this._canvas.height = render.height;
        //         } else {
        //             let ctx = this._canvas.getContext('2d');
        //             ctx.clearRect(0, 0, this._canvas.width, this._canvas.height);
        //         }
        //         let ctx = this._canvas.getContext('2d');
        //         let data = render.readPixels();
        //         let rowBytes = render.width * 4;
        //         for (let row = 0; row < render.height; row++) {
        //             let srow = render.height - 1 - row;
        //             let imageData = ctx.createImageData(render.width, 1);
        //             let start = srow * render.width * 4;
        //             for (let i = 0; i < rowBytes; i++) {
        //                 imageData.data[i] = data[start + i];
        //             }
        //             ctx.putImageData(imageData, 0, row);
        //         }
        //         var dataURL = this._canvas.toDataURL("image/png");
        //         var img = document.createElement("img");
        //         img.src = dataURL;
        //         let texture = new cc.Texture2D();
        //         texture.initWithElement(img);
        //         let spriteFrame = new cc.SpriteFrame();
        //         spriteFrame.setTexture(texture);
        //         let sprite = node.addComponent(cc.Sprite);
        //         sprite.spriteFrame = spriteFrame;
        //         node.parent = cc.director.getScene();
        //         node.runAction(spawn);

        //         var iframe = "<iframe width='100%' height='100%' src='" + dataURL + "'></iframe>"
        //         var x = window.open();
        //         x.document.open();
        //         x.document.write(iframe);
        //         x.document.close();
        //     }, 0, 0)
        // }
    },
    /**
     * 简单的复制对象
     * 正确处理的对象只有Number、String、Array等能够被json表示的数据结构，因此函数这种不能被json表示的类型将不能被正确处理
     * @param {mobject}  复制的对象原型
     */
    jsonCopy(mobject) {
        if (mobject) {
            return JSON.parse(JSON.stringify(mobject))
        }
    },
    /**
     * 设置默认头像
     * @param {head}  头像精灵
     */
    setDefaultHead(head) {
        let rand = Math.floor(Math.random() * 20)
        let randstr:string = rand.toString();
        if (rand < 10) {
            randstr = "0" + rand;
        }
        cc.resources.load("head/im_head" + randstr + "/spriteFrame", cc.SpriteFrame, function (err, res) {
            if (err) {
                return
            }
            if(cc.isValid(head))
            {
                head.spriteFrame = res;
            }
        })
    },
    /**
    *  字符串超长作固定长度加省略号（...）处理
    * @param str 需要进行处理的字符串，可含汉字
    * @param len 需要显示多少个汉字，两个英文字母相当于一个汉字
    * @returns {string}
    */
    formatStringLength(str, len = 12) {
        let strlen = 0;
        let needstr = str;
        for (let i = 0; i < str.length; i++) {
            if (str.charCodeAt(i) >= 19968 && str.charCodeAt(i) <= 40869) {
                strlen += 2;
            } else {
                strlen++;
            }
            if (strlen > len) {
                needstr = str.substr(0, i - 1) + "...";
                break;
            }
        }
        return needstr;
    },
    /**
     * 计算浮点型数值相减的结果
     * 精度最高为小数点后6位
     * @param value1
     * @param value2
     * @param precision 要求的小数点后精度,默认为小数点后4位
     */
    floatSub(value1, value2, precision = 4) {
        //精度值不能小于1
        precision = precision < 1 ? 4 : precision;
        //精度值不能大于6
        precision = precision >= 6 ? 6 : precision;
        //计算放大倍数
        let multiple = 10 ** precision;
        let buff1 = Math.floor(value1 * multiple);
        let buff2 = Math.floor(value2 * multiple);
        let result = (buff1 - buff2) + '';
        //还原精度
        let strHead = result.slice(0, -precision);
        let strEnd = result.slice(-precision);
        return +`${strHead}.${strEnd}`;
    },
    /**
     * 获取对应的本地日期
     * @param timestamp 本地时间戳
     * @returns 日期格式 2018年12月31日
     */
    toLocalDate(timestamp) {
        const date = new Date(1e3 * timestamp);
        const month = date.getMonth();
        const strMonth = (9 > month ? "0" + (month + 1) : month + 1).toString();
        const day = date.getDate();
        const strDay = (10 > day ? "0" + day : day).toString();
        return `${date.getFullYear()}年${strMonth}月${strDay}日`;
    },
    /**
     * 时间戳转化为时分秒的格式
     * @param timestamp 时间戳
     * @returns  日期格式 09:59:01
     */
    timestampHMS(timestamp) {
        const h = Math.floor(timestamp / 3600);
        const m = Math.floor(timestamp / 60 % 60);
        const s = Math.floor(timestamp % 60);
        return {
            hour: h < 10 ? "0" + h : "" + h,
            minute: m < 10 ? "0" + m : "" + m,
            second: s < 10 ? "0" + s : "" + s,
        };
    },
    padMonth(m: number): string {
        return m < 9 ? "0" + (m + 1) : "" + (m + 1);
    },
    padDay(d: number): string {
        return d < 10 ? "0" + d : "" + d;
    },
    /**
     * 年月日格式化为字符串
     * @returns `${y}年${i}月${o}日`
     */
    formatDateToString(y, m, d) {
        const strMonth = this.padMonth(m);
        const strDay = this.padDay(d);
        return `${y}年${strMonth}月${strDay}日`;
    },
    /**
     * 年月日格式化为数字
     */
    formatDateToNumber(y, m, d) {
        const i = this.padMonth(m);
        const o = this.padDay(d);
        return parseInt(`${y}${i}${o}`, 10);
    },
    formatDateToStr(timestamp) {
        let date = new Date(1e3 * timestamp)  // *1e3 转换时间
        let strYear = date.getFullYear();
        let month = date.getMonth();
        let strMonth = (9 > month ? "0" + (month + 1) : month + 1).toString();
        let day = date.getDate();
        let strDay = (10 > day ? "0" + day : day).toString();
        let hour = date.getHours();
        let strHour = (hour < 10 ? "0" + hour : hour).toString();
        let minute = date.getMinutes();
        let strMinute = (minute < 10 ? "0" + minute : minute).toString();
        let second = date.getMinutes();
        let strSecond = (second < 10 ? "0" + second : second).toString();
        let str = strYear + "-" + strMonth + "-" + strDay + " " + strHour + ":" + strMinute + ":" + strSecond
        return str
    },
    /**
     * 字符串数组转化为数字数组
     * @param strArray 需要转化的字符串数组
     */
    toNumberArray(strArray) {
        const intValues = [];
        const length = strArray.length;
        for (let index = 0; index < length; index++) {
            let val = parseInt(strArray[index], 10);
            if (!isNaN(val)) {
                intValues.push(val);
            }
        }
        return intValues;
    },
    moneyLevel: {
        Bai: 100,
        Qian: 1e3,
        Wan: 1e4,
        ShiWan: 1e5,
        BaiWan: 1e6,
        QianWan: 1e7,
        Yi: 1e8,
    },
    /**
     * 格式化金额为对应的字符串
     * @param money 需要格式化的金额
     */
    formatMoney(money) {
        if (money >= this.moneyLevel.Yi) {
            return Math.floor(money / this.moneyLevel.Yi) + "亿";
        }
        if (money >= this.moneyLevel.BaiWan) {
            return Math.floor(money / this.moneyLevel.Wan) + "万";
        }
        return this.fixedFloat(money, 2);
    },
    /**
     * 保留小数点后的指定位数，采用的方法是截断指定位数后的数字
     * @param num 需要操作的数字
     * @param len 需要保留的小数点后位数
     */
    fixedFloat(num, len = 2) {
        // return ~~(num * 100) / 100;
        if(num === 0 )return 0;
        return num.toFixed(len);
    },
    formatGold(num:number) {
        // num = num.toFixed(6)
        // return ~~(num * 100) / 100;
        num = Number(num.toFixed(6));
        if(num < 0.01 && num > 0 ){
            num = 0;
        }
        let str = num.toString()
        if (str.includes(".")) {
            return str.substring(0, str.indexOf(".") + 3)
        } else {
            return str
        }
    },
    formatGold2(num:number , len = 4 ) {
        let str = num.toString()
        if (str.includes(".")) {
            str = str.substring(0, str.indexOf(".") + len )
            while( str.length - 1 > str.indexOf(".") && str[str.length-1] === '0' ){
                str = str.substring(0,str.length-1);
            }
            if(str.length -1 === str.indexOf(".")){
                str = str.substring(0,str.length-1);
            }
        }
        return str;
    },
    
    /**
     * 填充数字，对于长度不足的，自动补零
     * @param num 需要填充的数字
     * @param len 指定的长度
     */
    padNumber(num, len) {
        const n = ("" + num).length;
        return Array(len > n ? len - n + 1 || 0 : 0).join("0") + num;
    },
    /**
     * @Description: 互换数组中的两个元素
     */
    swapItem(arr, fromIndex, toIndex) {
        arr[toIndex] = arr.splice(fromIndex, 1, arr[toIndex])[0];
        return arr;
    },
    // 将int转换为uint8数组
    IntToUint8Array(num, Bits) {
        let resArry = [];
        let xresArry = [];
        let binaryStr = num.toString(2);
        for (let i = 0; i < binaryStr.length; i++)
            resArry.push(parseInt(binaryStr[i]));

        if (Bits) {
            for (let r = resArry.length; r < Bits; r++) {
                resArry.unshift(0);
            }
        }

        let resArryStr = resArry.join("");
        for (let j = 0; j < Bits; j += 8)
            xresArry.push(parseInt(resArryStr.slice(j, j + 8), 2));

        return xresArry;
    },
    // 将unit8数组转换为int..
    Uint8ArrayToInt(uint8Ary) {
        let retInt = 0;
        for (let i = 0; i < uint8Ary.length; i++)
            retInt |= (uint8Ary[i] << (8 * (uint8Ary.length - i - 1)));

        return retInt;
    },
    // 全局设置所有按钮点击后延时
    setButtonDelay(b, t) {
        if (b) {
            if (!cc.Button.prototype.tocheEndClose) {
                cc.Button.prototype.tocheEndClose = cc.Button.prototype._onTouchEnded
            }
            cc.Button.prototype._hqqDelay = true;
            cc.Button.prototype._hqqDelayTime = t || 500;
            cc.Button.prototype.hqqsetDelay = function (on) {
                if (typeof on == 'undefined') {
                    this._hqqDelay = true
                } else {
                    this._hqqDelay = on
                }
            }
            cc.Button.prototype.hqqDelay = function (event) {
                if(!cc.isValid(event.target))return;
                if (this.interactable && this.enabledInHierarchy && this._hqqDelay) {
                    this.enabledInHierarchy = false // enabledInHierarchy 表示该组件是否被启用并且所在的节点也处于激活状态。
                    setTimeout(() => {
                        this.enabledInHierarchy = true
                    }, this._hqqDelayTime);
                }
            }
            cc.Button.prototype._onTouchEnded = function (event) {
                if(!cc.isValid(event.target))return;
                if (this._hqqSoundon) {
                    this.hqqEffect()
                }
                this.tocheEndClose(event);
                if (this._hqqDelay) {
                    this.hqqDelay(event)
                }
            }
        } else {
            cc.Button.prototype._hqqDelay = false
        }
    },
    formatDateTime(time: any) {
        // 格式化日期，获取今天的日期
        const Dates = new Date(time);
        let hour = Dates.getHours();
        let strHour = (hour < 10 ? "0" + hour : hour).toString();
        let minute = Dates.getMinutes();
        let strMinute = (minute < 10 ? "0" + minute : minute).toString();
        let second = Dates.getSeconds();
        let strSecond = (second < 10 ? "0" + second : second).toString();
        return this.formatDate(time) + ' ' + strHour + ':' + strMinute + ':' + strSecond;
    },

    formatDate(time: any) {
        // 格式化日期，获取今天的日期
        const Dates = new Date(time);
        const year: number = Dates.getFullYear();
        const month: any = (Dates.getMonth() + 1) < 10 ? '0' + (Dates.getMonth() + 1) : (Dates.getMonth() + 1);
        const day: any = Dates.getDate() < 10 ? '0' + Dates.getDate() : Dates.getDate();
        return year + '-' + month + '-' + day;
    },
    //跟据两点返回角度
    CalAngel(sx: number, sy: number, tx: number, ty: number) {
        const disX = tx - sx;
        const disY = ty - sy;

        //通过两点距离求反正切
        const nRadian = Math.atan2(disY,disX);
        let nAngel = (Math.PI*2 - nRadian) * (180/Math.PI);

        //let nAngel = nRadian * (180/Math.PI);

        if (nAngel >= 360) {
            nAngel -= 360;
        }

        return nAngel;
    },
     /**
    * @param {number} range
    * @param {string} [type]
    * @memberOf VehicleOverviewComponent
    * @description 获取今天及前后天
    */
     getRangeDate(range: number, type?: string) {
        const now = this.formatDate(new Date().getTime()); // 当前时间
        const resultArr: Array<any> = [];
        let changeDate: string;
        if (range) {
            if (type) {
                if (type === 'one') {
                    changeDate = this.formatDate(new Date().getTime() + (1000 * 3600 * 24 * range));
                    return changeDate;
                }
            } else {
                changeDate = this.formatDate(new Date().getTime() + (1000 * 3600 * 24 * range));
                return changeDate;
            }
        } else {
            return now;
        }
    },

    /**
     * 
     * @param str 要加上省略號的文字
     * @param len 指定从第几个字开始加上省略号
     */
    addellipsis(str:string,len:number = 9){
        let tempstr = str;
        let strlength = 0;
        let strlength2 = 0;
        let i = 0;
        let isaddellipsis = false;
        for (i = 0; i < tempstr.length; i++) {
            let a = tempstr.charAt(i);
            let num = 1;
            let pattern = new RegExp("[\\u4E00-\\u9FFF]+","g");
            if( pattern.test(a) ){
                num++;
            }
            if(strlength + num <= len ){
                strlength += num;
                strlength2++;
            } else{
                isaddellipsis = true;
                break;
            }
        }
        i = null;
        if(isaddellipsis){
            return tempstr.substring(0,strlength2)+".";
        } else{
            return tempstr;
        }
    },
}
