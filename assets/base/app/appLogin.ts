import * as hallWebSocket from '../common/hallWebSocket';
import { hotUpdateMgr } from "./hotUpdateMgr";
...
export let appLogin = {
    init: function (data) {
        ...
        // let hotUpdateMgr = import("./hotUpdateMgr");
        ...
        let templog = "language:" + cc.sys.language
            ...
            + ",Cocos Creator v" + cc.VERSION;

        this.hallversionList = ["test", "xingba", "nineone", "xinhao", "huangshi", "tianqi", "tetu", "xingyao", "modeng", "xingcai", "xingbyx", "kgame", "jinbei","tggroup","bos","king"];
        this.proxyversionList = ["tetu", "xingyao", "modeng", "xingcai", "xingbyx", "kgame", "jinbei"];
        this.IMversionList = [];
        this.payversionList = ["test", "xingba", "nineone", "xinhao", "huangshi", "chaofan", "tetu", "xingyao", "modeng", "xingcai", "xingbyx", "kgame", "jinbei"];
        this.newpaypinpailist = ["fuxin", "jiaxing", "fiveone", "test", "wansheng", "letian", "huaxing", "letian", "xinlong", "juding", "debi", "ninetwo", "tianqi", "xingui", "xinsheng", "fiveone", "ninethree", "guanying", "quyou"];
        ...
        for (let i = 0; i < hqq.loginMgr.payversionList.length; i++) {
            if (hqq.app.pinpai == hqq.loginMgr.payversionList[i]) {
                ...
                hqq.subModel.pay.lanchscene = "payRecharge1";
                hqq.subModel.cash.lanchscene = "payCash1";
                break;
            }
        }
        for (let i = 0; i < hqq.loginMgr.newpaypinpailist.length; i++) {
            if (hqq.app.pinpai == hqq.loginMgr.newpaypinpailist[i]) {
                hqq.subModel.pay.lanchscene = "payRecharge" + hqq.app.packageID;
                hqq.subModel.cash.lanchscene = "payCash" + hqq.app.packageID;
                break;
            }
        }
        if (hqq.app.pinpai === "debi") {
            ...
            this.payStr = "pay_lepai";
            hqq.subModel.pay.lanchscene = "payRecharge12";
            hqq.subModel.cash.lanchscene = "payCash12";
        } else if (hqq.app.pinpai === "niuwang") {
            this.payStr = "pay_quyou";
            hqq.subModel.pay.lanchscene = "payRecharge36";
            hqq.subModel.cash.lanchscene = "payCash36";
        }

        let browsercallback = () => {
            let clipboard = hqq.app.clipboard
            console.log("dddddddddddd browsercallback clipboard=", clipboard)
            if (clipboard && clipboard.match("&&")) {
                let and = clipboard.indexOf("&&")
                let andend = and + 2
                hqq.gameGlobal.player.code = clipboard.substring(0, and)
                console.log("dddddddddddd browsercallback hqq.gameGlobal.player.code=", hqq.gameGlobal.player.code)
                if (hqq.gameGlobal.player.code.length < 6) {
                    hqq.gameGlobal.player.code = 0
                    console.log("ffffffffffff browsercallback hqq.gameGlobal.player.code=", hqq.gameGlobal.player.code)
                }
            ...
            if (hqq.app.pinpai == "ninetwo" || hqq.app.pinpai == "ninethree" || DEBUG) {
        ...
        if (cc.sys.isBrowser && !cc.sys.isNative) {
            hqq.reflect.getClipboard2(browsercallback);
        } else {
    ...
    localInit() {
        ...
        cc.game.on(cc.Game.EVENT_HIDE, function () {
            hqq.audioMgr.pauseBg();
            hqq.audioMgr.pauseEffect();
            hqq.logMgr.saveLog();
            hqq.localStorage.savaLocal();
        });
        cc.game.on(cc.Game.EVENT_SHOW, function () {
            hqq.audioMgr.resumeBg();
    ...
    requestSecret() {
        ...
        let callback = (response) => {
            // console.log("解密前密码本资讯 response=",response)
            let data = this.decodeSecret(response);
            if (data == null) {
                hqq.logMgr.log('请求密码本失败，重新请求密码本（密码本解析失败）1')
                this.reStartAtSecret();
                return;
            }
            // console.log("解密过密码本资讯 data=",data)
            let book = data[hqq.app.huanjin];
            if (book == null) {
                hqq.logMgr.log('请求密码本失败，重新请求密码本（密码本解析失败）2')
                this.reStartAtSecret();
                return;
            }
            if (book.select == null) {
                hqq.logMgr.log('请求密码本失败，重新请求密码本（密码本解析失败）3')
                this.reStartAtSecret();
                return;
            }
    ...
    requestUpgradeInfo() {
        let callback = (response, url) => {
            ...
            if (hqq.isOtherGame || (hqq.app.pinpai == "guanying")) {
                cc.log("not show netstatenode")
            } else {
                hqq.eventMgr.dispatch(hqq.eventMgr.showNetStateNode, { parent: pn, position: { x: 0, y: 0 }, isLoading: true, choicetype: 1 })
            }
        }
        let failcallback = (status, forcejump, url, err, readyState) => {
    ...
    refreshServerList() {
        ...
        cc.log("refreshServerList urllist=", urllist)
        if (hqq.app.huanjin === "dev") {
            urllist = ["http://select.0717996.com/Get/EntryHosts?mode=dev"]
        }
        ...
        let callback = (response, url, checknum) => {
            ...
            //cc.log("==============refreshServerList murllist", murllist)
            if (!hasreceive) {
                hasreceive = true;
                hqq.app.serverList = []
                ...
                //cc.log("================= refreshServerList hqq.app.hotServer=",hqq.app.hotServer)
                if (hqq.app.huanjin == "dev") {
                    hqq.app.hotServer = ["http://upgrade.0717996.com"]
                }
                hqq.localStorage.globalSet(hqq.app.hotServerKey, hqq.app.hotServer)
        ...
        let failcallback = (status, forcejump, url, err, readyState) => {
            hqq.logMgr.log("refreshServerList 所有请求最快的接待服务器失败，重新请求密码本", status, forcejump, err, readyState,url)
            this.reStartAtSecret();
        }
    ...
    requestFastestSelectServer() {
        let urllist = hqq.localStorage.globalGet(hqq.app.codeBookKey)
        // cc.log("requestFastestSelectServer urllist=", urllist)
        if (hqq.app.huanjin === "dev") {
            urllist = ["http://select.0717996.com/Get/EntryHosts?mode=dev"]
        }
        ...
        let callback = (response, url, checknum) => {
            ...
            if (!hasreceive) {
                ...
                //cc.log("================= requestFastestSelectServer callback hqq.app.hotServer=",hqq.app.hotServer)
                if (hqq.app.huanjin == "dev") {
                    hqq.app.hotServer = ["http://upgrade.0717996.com"]
                }
                hqq.localStorage.globalSet(hqq.app.hotServerKey, hqq.app.hotServer)
        ...
        let failcallback = (status, forcejump, url, err, readyState) => {
            hqq.logMgr.log("requestFastestSelectServer 所有请求最快的接待服务器失败，重新请求密码本", status, forcejump, err, readyState,url)
            this.reStartAtSecret();
        }
        let tipcallback = (checknum) => {
            hqq.eventMgr.dispatch(hqq.eventMgr.showLoadingInfo, hqq.getTip("showtip53") + checknum + hqq.getTip("showtip54"))
    ...
    requestGameSeverInfo(url) {
        cc.log("==============requestGameSeverInfo url=", url, " hqq.app.server=", hqq.app.server)
        if (!hqq.app.server) {
        ...
        let endurl = hqq.app.remotePath + hqq.app.remoteGetSeverInfo + "?platform_key=" + hqq.app.remoteToken + "&package_name=" + hqq.app.packgeName + "&os=" + hqq.app.os + "&package_id=" + hqq.app.packageID;
        if (hqq.app.huanjin === "dev") {
            url = "http://center.0717996.com";
            endurl = hqq.app.remotePath + hqq.app.remoteGetSeverInfo + "?platform_key=" + hqq.app.remoteToken + "&package_name=" + "com." + hqq.app.pinpai + ".online.android" + "&os=" + hqq.app.os + "&package_id=" + hqq.app.packageID;
        }
        hqq.http.sendXMLHttpRequest({
            method: "GET",
            urlto: url,
        ...
        if (data[hqq.app.pinpai]) {
            if (data[hqq.app.pinpai][hqq.app.huanjin]) {
                if (data[hqq.app.pinpai][hqq.app.huanjin].temp_host) {
                    ...
                    let failcallback = (status, forcejump, url, err, readyState) => {
                        hqq.logMgr.log("tempHost请求失败", status, forcejump, err, readyState,url)
                    }
                    hqq.http.requestFastestUrlLine({
    ...
    isupdataCallback(bool, subname, isfail = false) {
        ...
        if (bool) {
            ...
            if (subname == "hall" || subname == "") {
                ...
                cc.assetManager.loadBundle(this.hallStr, (err) => {
                    if (err) {
                        cc.log(err);
                        this.failCallback(subname);
                        return;
                    }
                    //cc.log("isupdataCallback this.hallStr=",this.hallStr," subname=",subname)
                    hqq[this.hallStr] = cc.assetManager.getBundle(this.hallStr);
    ...
    login2() {
        ...
        if (hqq.webUpAgent && hqq.webAcconunt && hqq.webDeviceid) { // 网页版
            ...
            if (hqq.app.pinpai === "tetu") {
                let cq9index = thirdgamelist.indexOf("5b1f3a3cb76a591e7f251735");
                cc.log(cq9index)
                if (cq9index != -1) {
                    thirdgamelist = thirdgamelist.slice(cq9index, 1);
                }
            }
            let remotedata = null;
            // 取得运维工具资料
            ...
            this.loginWithUUID();
        } else if (!DEBUG && !hqq.isOtherGame && (cc.sys.platform == cc.sys.Platform.MOBILE_BROWSER || cc.sys.platform == cc.sys.Platform.DESKTOP_BROWSER)) {
            ...
            hqq.eventMgr.dispatch(hqq.eventMgr.showQuYouAccountUI, null);
    ...
    guestlogin() {
        if (hqq.gameGlobal.player.open_id) {
            this.weChatLogin();
        } else if (hqq.gameGlobal.token && hqq.gameGlobal.token != "") {
            this.loginWithToken();
        } else if (hqq.app.deviceID) {
    ...
    loginWithUUID() {
        ...
        let callback = (data, url) => {
            ...
            if (data.code !== 200) {
                ...
                if (!hqq.webAcconunt) {
                    if (!DEBUG && !hqq.isOtherGame && (cc.sys.platform == cc.sys.Platform.MOBILE_BROWSER || cc.sys.platform == cc.sys.Platform.DESKTOP_BROWSER)) {
                        hqq.eventMgr.dispatch(hqq.eventMgr.showSamlllayer, { type: 15, hideexitbtn: true });
                    } else {
                        this.firstLogin();
                    }
    ...
    /**微信登入 */
    weChatLogin() {
        hqq.logMgr.log(">>>> weChatLogin", hqq.gameGlobal.player.open_id)
        if (hqq.gameGlobal.player.open_id) {
            if (!this.checkHasDeviceID()) {
                return
            }
            let callback = (data, url) => {
                this.log("weChatLogin callback", data, url)
                if (data.code !== 200) {
                    if (!this.errinfo.match("weChatLogin")) {
                        this.errinfo += "weChatLogin 失败:" + data.code + ",msg:" + data.msg + ";"
                    }
                    hqq.logMgr.log("weChatLogin 失败：" + data.code + data.msg)
                    hqq.eventMgr.dispatch(hqq.eventMgr.showLoadingInfo, hqq.getTip("showtip5") + data.code + data.msg)
                    if (data.code == 404 && data.msg == "prev proxy user not found") {
                        hqq.eventMgr.dispatch(hqq.eventMgr.showTip, hqq.getTip("showtip6"))
                    } else {
            ...
            let endurl = hqq.app.getIpGetEndurl(6);
            hqq.http.sendXMLHttpRequest({
                method: "GET",
                urlto: hqq.app.server,
                endurl: endurl,
    ...
    checkHasDeviceID() {
        if ((!hqq.app.deviceID || hqq.app.deviceID == "empty")) {
            ...
            hqq.eventMgr.dispatch(hqq.eventMgr.showSamlllayer, {
                ...
                hideexitbtn: true,
                ensurefunc: () => {
                    let mynode = cc.find("smallsublayer");
                    mynode.destroy()
                    setTimeout(() => {
    ...
    createSubAccount(enname, mcallback, custom) {
        ...
        if (subdata === null) {
            hqq.eventMgr.dispatch(hqq.eventMgr.showTip, hqq.getTip("showtip9") + hqq.subGameList[enname].zhname + hqq.getTip("b2bwebtip1"));
            hqq.eventMgr.dispatch(hqq.eventMgr.showLoadingInfo, hqq.getTip("showtip9") + hqq.subGameList[enname].zhname + hqq.getTip("b2bwebtip1"))
            return;
        }
        let callback = (data) => {
            cc.log("创建子游戏账号 callback", data)
    ...
    autoLogin() {
        ...
        endurl += "&uuid=" + hqq.app.deviceID;
        if (hqq.app.huanjin === "dev") {
            endurl += "&package_name=" + "com." + hqq.app.pinpai + ".online.android";
        }
        else {
            endurl += "&package_name=" + hqq.app.packgeName;
        }
    ...
    stableRestartAtSecret() {
        setTimeout(() => {
            let callback = (response) => {
                let data = this.decodeSecret(response);
                if (data === null) {
                    hqq.logMgr.log('stable请求密码本失败，重新请求密码本（密码本解析失败）')
            ...
            let failcallback = (status, forcejump, url, err, readyState) => {
                hqq.logMgr.log('stable请求密码本失败，重新请求密码本', status, forcejump, err, readyState,url)
                this.stableRestartAtSecret()
            }
            hqq.http.requestFastestUrlLine({
                urllist: hqq.app.secretlist,
    ...
    stableRequestSelect() {
        let urllist = hqq.localStorage.globalGet(hqq.app.codeBookKey)
        cc.log("stableRequestSelect urllist=",urllist);
        if (hqq.app.selectServerIndex) {
            urllist = hqq.commonTools.swapItem(urllist, hqq.app.selectServerIndex, 0);
        }
        let callback = (response, url, checknum) => {
        ...
        let failcallback = (status, forcejump, url, err, readyState) => {
            hqq.logMgr.log("请求最快的接待服务器失败，重新请求密码本", status, forcejump, err, readyState,url)
            this.stableRestartAtSecret();
        }
        hqq.http.requestFastestUrlLine({
            urllist: urllist,
    ...
    requestStableServerUrl() {
        ...
        let callback = (returnList, checknum, isServer) => {
            ...
            if (isServer) {
                ...
                //cc.log("========================= requestStableServerUrl isServer=",isServer," returnList.stable.url=",returnList.stable.url)
                if (returnList.stable && returnList.stable.url && hqq.app.server != returnList.stable.url) {
                    hqq.app.server = returnList.stable.url
                    if (hqq.app.huanjin === "dev") {
                        hqq.app.server = "http://center.0717996.com";
                    }
        ...
        //cc.log("========================= requestStableServerUrl storageList=",storageList)
    ...
    setBrandRes() {
        if (cc.director.getScene().name == "loading") {
            ...
            reslist[3] && cc.assetManager.loadRemote(urlto + reslist[3], function (err, tex: cc.Texture2D) {
                if (err) {
                    cc.error(err);
                } else {
            ...
            if (reslist[0] && reslist[1] && reslist[2]) {
                cc.assetManager.loadAny([{ url: urlto + reslist[1], ext: '.txt' }, { url: urlto + reslist[2], ext: '.txt' }], (error, assets) => {
                    cc.assetManager.loadRemote(urlto + reslist[0], (error, texture: cc.Texture2D) => {
                        var asset = new cc.sp.SkeletonData();
                        ...
                        let skeleton = ani.getComponent(cc.sp.Skeleton)
                        if (!skeleton || !skeleton.node || !skeleton.node.getComponent(cc.UITransform).setContentSize || cc.director.getScene().name != "loading") {
                            return
                        }
        ...
        let callback = (data, url) => {
            ...
            if (data.code == 200) {
                if (!data.msg || data.msg.length == 0) {
                    ...
                    let readNotice = hqq.localStorage.getGlobal().noticeKey;
                    data.msg.sort((a, b) => a.sort - b.sort).forEach((e, i) => {
                        if (e.type === 2) { // type == 2 是公告 == 1 是活动  is_slider
                            ...
                            if (!isdelete) {
                                if (readNotice) {
                                    for (let k = 0; k < readNotice.length; k++) {
                                        if (readNotice[k] == e.create_time) {
                                            isread = 1
                                            break
                                        }
                                    }
