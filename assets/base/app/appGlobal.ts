...
export let appGlobal:appGlobalInterface = {
    ...
    subGameVersion: null, // version.json 中的version部分
    proxyUserID: 0, // 代理id
    packageID: 0, // 包体id
    netState: { // 网络状态分级
        outstanding: 500,
    ...
    init(sys) {
        ...
        if( this.GeneralAgency[this.pinpai] )
        {
            ...
            {
                GeneralAgency=this.GeneralAgency[this.pinpai][this.huanjin]
            }
        }

        let temppackageinfo = hqq.reflect.getHqqPackageInfo(this.pinpai,this.huanjin,this.apkVersion,GeneralAgency)
        if (temppackageinfo) {
        ...
        if (this.pinpai == "test") {
            ...
            this.packageID = 21
        } else if (this.pinpai == "chaofan" || this.pinpai == "lantian") {
            this.packageID = 19
            ...
            this.packageID = 29
        } else if (this.pinpai == "modeng") {
            this.packageID = 30
        } else if (this.pinpai == "xingcai") {
            this.packageID = 31
        } else if (this.pinpai == "xingbyx") {
            this.packageID = 32
        } else if (this.pinpai == "kgame") {
            this.packageID = 33
        } else if (this.pinpai == "guanying") {
            this.packageID = 35
        } else if (this.pinpai == "quyou") {
            this.packageID = 36
        } else if (this.pinpai == "jinbei") {
            this.packageID = 37
        } else if (this.pinpai == "jinma") {
            this.packageID = 38
        } else if (this.pinpai == "pusheng") {
            this.packageID = 39
        } else if (this.pinpai == "xinyinhe") {
            this.packageID = 40
        } else if (this.pinpai == "fucaiyule") {
            this.packageID = 41
        } else if (this.pinpai == "yunding") {
            this.packageID = 42
        } else if (this.pinpai == "fuwang") {
            this.packageID = 43
        } else if (this.pinpai == "fuhui") {
            this.packageID = 45
        } else if (this.pinpai == "xingxing") {
            this.packageID = 46
        } else if (this.pinpai == "tggroup") {
            this.packageID = 47
        } else if (this.pinpai == "niuwang") {
            this.packageID = 48
        } else if (this.pinpai == "bos") {
            this.packageID = 49;
        } else if (this.pinpai == "king") {
            this.packageID = 50;
        }     
        
        this.b2blist = ["tetu","xingyao","modeng","xingcai","xingbyx","kgame","jinbei","tggroup","bos","king"];
        ...
        if (hqq.isDebug) {
            ...
            this.deviceID = "ray202303211140" // burt
        }
        return this;
    },
    // 获取向中心服get的接口及组合参数
    getIpGetEndurl(gettype) {
        ...
        switch (gettype) {
            ...
            case 4:
                ...
                endurl += "&id=" + this.gameUser.id;
                endurl += "&platform_key=" + this.remoteToken;
                let query = JSON.stringify({
                    'package_ids': { $elemMatch: { $eq: this.gameUser.package_id } },
                    'is_open': 1,
                    'start_time': { $lte: time },
                    'end_time': { $gte: time }
                })
            ...
            case 5:
                ...
                endurl += "&id=" + this.gameUser.id;
                endurl += "&token=" + this.gameGlobal.token;
                return endurl;
            case 6:
                endurl += "/Game/Login/weChatLogin?" // 微信登陆
                break;
        }
        ...
        if(gettype == 6)// 微信登陆訊息
        {
            endurl += "&open_id=" + hqq.gameGlobal.player.open_id;
            endurl += "&nick_name=" + hqq.gameGlobal.player.nick_name;
            endurl += "&head_img_url=" + hqq.gameGlobal.player.head_img_url;
        }
        return endurl
    },
    ...
    setGeneralAgency(data) {
        ...
        for (let huanjin in data[this.pinpai]) {
            if(data[this.pinpai][huanjin].proxyUserID){
    ...
    // 根据中心服返回设置游戏相关信息
    setGameInfo(game_user, proxy_user=null, prev_proxy=null) {
        let data = { 
        ...
        if (game_user) {
            for (let k in game_user) {
                this.gameUser[k] = (this.gameUser && this.gameUser[k]) || game_user[k]
            }
            if (game_user.id) {
            ...
            if (game_user.bank_gold || game_user.bank_gold == 0) {
                let bankgold = game_user.bank_gold
                if (bankgold < 0.01) {
                    bankgold = "0";
                } else {
                    bankgold = hqq.commonTools.formatGold(game_user.bank_gold);
                }
                hqq.gameGlobal.player.bankgold = bankgold;
                data.bankgold = bankgold
            }

            if (game_user.game_img) {
                let header = game_user.game_img.split("://")[0];
    ...
    isB2BList(){
        if( this.b2blist.indexOf(hqq.app.pinpai) != -1 ){
            return true;
        }
        return false;
    },
