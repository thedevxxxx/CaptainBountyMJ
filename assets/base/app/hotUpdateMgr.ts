...
// import { Buffer as nodeJsBufferTool }  from "buffer";
export let hotUpdateMgr = {
    manifestPre: "",
    _failCount: 0,
    _data: {
        subname: "",
        ...
        switch (event.getEventCode()) {
            ...
            case jsb.EventAssetsManager.ERROR_UPDATING:  // 7
                ...
                if (this._upFailNumTemp >= this._upFailNum) {
                    ...
                    console.log("up更新错误 event=",event," getDownloadedFiles=",event.getDownloadedFiles(), " getDownloadedBytes=",event.getDownloadedBytes()," getTotalFiles=",event.getTotalFiles()," getTotalBytes=",event.getTotalBytes(), " getPercent=",event.getPercent()," getPercentByFile=",event.getPercentByFile());
                    this.log("up更新错误," + this._failCount + "次,", event.getAssetId(), event.getMessage(),
                        "curleCode", event.getCURLECode(), "curlmCode", event.getCURLMCode(),"  event=",event)
                }
                break;
            case jsb.EventAssetsManager.UPDATE_FINISHED:  // 8
                this.log("up更新完成")
    ...
    checkSubGame(checkManifest, callback, data) {
        ...
        if (check.bool) {
            let remotestr:any = jsb.fileUtils.getStringFromFile(checkManifest)
    ...
    _loadCustomManifest: function (subname, verstr) {
        ...
        if (jsb.fileUtils.isFileExist(localpath)) {
            ...
            if (subname == 'hall' && this.manifestData.hallmanifest) {
                ...
                this.log("url", url + this.manifestPre + "project.manifest" , " this.data.version=",this.data.version)
    ...
    calculateMD5(assetpath) {
        ...
        const arrayBuffer: ArrayBuffer = dataBinary;
        let sirData = new Uint8Array(arrayBuffer);
        // let localmd5 = crypto.createHash('md5').update(sirData).digest('hex'); // 与打包时的MD5码计算一致 CRLF LF 变化需要注意，会造成MD5不同
        let localmd5 = CryptoES.MD5(CryptoES.lib.WordArray.create(sirData)).toString(CryptoES.enc.Hex);
        return localmd5
    },
    /**
    ...
    startUpdate(packageUrl) {
        let callback = (responseText) => {
            ...
            this.log('loadRemoteManifest', this._am.loadRemoteManifest(manifest) , " customManifestStr=",customManifestStr," this._tempStoragePath=",this._tempStoragePath)
        }
