...
public class NativeAndroidClass extends  CocosActivity {
    //获取设备唯一标识
    public static String getUniqueIdAction() {
        // Context context = GlobalObject.getActivity();
        String uuId = Settings.Secure.getString(GlobalObject.getActivity().getContentResolver(), Settings.Secure.ANDROID_ID);
        return  uuId;
    }
    ...
    public static boolean savePicture(String base64DataStr) {
        ...
        try {
           ...
           return  true;
            ...
            intent.setData(uri);
            GlobalObject.getActivity().sendBroadcast(intent);
            Toast.makeText(GlobalObject.getActivity(),"图片保存成功",Toast.LENGTH_SHORT);
        }
    }
